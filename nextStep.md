...
Keep narrowing the remaining shell-local OCCT bbox fallback in `offset_shell_bbox()`, but stay on the shell-boundary Rust path. The next bounded Rust-first cut is to collapse the remaining duplicated direct `ControlFlow::Continue(samples)` vs `ControlFlow::Break(result)` carry inside the shared two-stage early probe runner, so midpoint-stage, outer-stage, and interval-aware dispatch stay on one smaller Rust-owned boundary without handwritten stage-to-stage carry twice in the same function.
...
- the midpoint-stage and outer-stage request/sample mapping now live in the same typed stage layout
  ...
  - but the staged early-probe path still spells the same `ControlFlow::Continue(samples)` vs `ControlFlow::Break(result)` carry twice by hand inside [`EarlyProbeRefinementStages::needs_refinement()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs): once from midpoint-stage into outer-stage, and once from outer-stage into interval-aware dispatch

The next blocker is to keep that typed `ControlFlow` progress path but collapse the remaining duplicated stage carry into one smaller runner-owned boundary, so midpoint-stage -> outer-stage progression and interval-aware dispatch stay in the same Rust-owned path without repeating the same direct `Continue`/`Break` handoff twice.
...
This turn removed the last one-use continuation helpers around the early-probe runner, so midpoint-stage, outer-stage, and interval-aware dispatch are all now typed through the same direct path. That leaves the next real seam smaller again: the behavior is already shared, but the runner still hand-writes the same `Continue(samples)` vs `Break(result)` carry twice before it reaches the interval-aware segment chooser.
