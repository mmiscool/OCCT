...
Keep narrowing the remaining shell-local OCCT bbox fallback in `offset_shell_bbox()`, but stay on the shell-boundary Rust path. The next bounded Rust-first cut is to keep the typed midpoint stage, typed outer-stage tail, and typed interval-aware side layouts in place while collapsing the now-thin `EarlyProbeStagePair::needs_refinement()` forwarder.
...
- the midpoint-stage and outer-stage request/sample mapping now live in the same typed stage layout
  ...
  - [`EarlyProbeStagePair`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now already holds midpoint-stage plus a typed [`EarlyProbeOuterStageTail`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs), and the top-level early probe entry now hands it a concrete `[NormalizedEdgeSample; 3]` source boundary directly
  ...
  - but the early probe pair still has one smaller structural seam: [`EarlyProbeStagePair::needs_refinement()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) is now only a thin forwarder from midpoint-stage into the typed [`EarlyProbeOuterStageTail`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs), so the remaining cleanup is collapsing that one-use pair wrapper instead of moving more execution back onto the pair

The next blocker is to keep the typed midpoint stage, typed outer-stage tail, and typed interval-aware side layouts, but collapse the now-thin pair forwarder so midpoint-stage to interval-aware progression sits on one smaller Rust-owned boundary without reintroducing the old deleted wrappers.
...
This turn moved the inline midpoint-stage -> outer-stage -> interval-aware chain onto the typed `EarlyProbeOuterStageTail`. That leaves one smaller seam in this slice: `EarlyProbeStagePair::needs_refinement()` is now just a thin midpoint-stage forwarder into that typed tail.
