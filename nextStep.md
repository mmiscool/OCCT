...
Keep narrowing the remaining shell-local OCCT bbox fallback in `offset_shell_bbox()`, but stay on the shell-boundary Rust path. The next bounded Rust-first cut is to collapse the remaining one-use midpoint-stage to outer-stage `and_then(...).transpose().map(Option::flatten)` carry inside `EarlyProbeStagePair::needs_refinement()`, so the typed two-stage early probe progression can sit on one Rust-owned boundary without that last functional stage-pair bounce.
...
- the midpoint-stage and outer-stage request/sample mapping now live in the same typed stage layout
  ...
  - but [`EarlyProbeStagePair::needs_refinement()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) still carries midpoint-stage into outer-stage through a one-use `and_then(...).transpose().map(Option::flatten)` bounce before interval-aware dispatch

The next blocker is to keep the typed stage-pair path but collapse the remaining one-use midpoint-stage to outer-stage `and_then(...).transpose().map(Option::flatten)` bounce inside `EarlyProbeStagePair::needs_refinement()`, so the two-stage early probe progression can stay on one Rust-owned boundary without that last functional carry.
...
This turn removed the pair-level `Result<Option<bool>, bool>` wrapper from the early stage pair by moving final `Err(result) => Some(result)` and `Ok(None) => None` handling onto `PreparedIntervalAwareRefinementSideLayouts::needs_refinement_from_stage_samples_or_result()`. That leaves the next real seam smaller again: `EarlyProbeStagePair::needs_refinement()` still carries midpoint-stage into outer-stage through a one-use `and_then(...).transpose().map(Option::flatten)` bounce before interval-aware dispatch.
