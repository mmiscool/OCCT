...
Keep narrowing the remaining shell-local OCCT bbox fallback in `offset_shell_bbox()`, but stay on the shell-boundary Rust path. The next bounded Rust-first cut is to collapse the remaining raw positional `source_index()` remap in the early shell-edge refinement path, so midpoint-stage and outer-stage typed source-slot enums stop each translating their slots back into array indices separately.
...
- but the typed source-slot enums still duplicate the final raw positional remap one level deeper: midpoint-stage and outer-stage each still keep their own `source_index()` match that translates typed slots back into array positions

The next blocker is to collapse that last raw positional `source_index()` remap into one shared typed boundary, so midpoint-stage and outer-stage source-slot descriptors stop each carrying their own array-position lookup match.
...
This turn finished the shared typed early-stage sample-role cut: midpoint-stage and outer-stage now reuse the same generic `EarlyProbeSampleRole` / `EarlyProbeSampleLayout` path, and the duplicated stage-local `source sample` / `stage sample` ladders are gone. That leaves the next real seam one level smaller again: the typed source-slot enums still duplicate the final slot-to-index remap in their own `source_index()` matches.
