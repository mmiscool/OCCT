...
Keep narrowing the remaining shell-local OCCT bbox fallback in `offset_shell_bbox()`, but stay on the shell-boundary Rust path. The early unsupported-edge probe-refinement entry is now carrier-owned end to end, and the winning interval-aware `RefinementSegment` now owns its own score-based creation, stronger-segment choice, local-window test, and stronger-half chase. The next bounded Rust-first cut is to collapse the remaining interval-aware `left/right` split and side-specific `outer`/`inner` segment assembly in `PreparedOuterProbeChain`, so the outer carrier stops branching over tuple-shaped segment prep inline.
...
The remaining duplication is no longer the raw early probe chain, it is no longer the stage wrappers, it is no longer the interval-aware staging carrier, and it is no longer the open-coded execution step on the prepared segment. The remaining structural gap is now the interval-aware side-specific segment assembly in [`PreparedOuterProbeChain`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs). That carrier still has to:

- call its separate `interval_aware_refinement_side_is_right()` chooser
- branch on that boolean to build side-specific `outer_start/outer_mid/outer_end` and `inner_start/inner_end` tuples
- then create the competing interval-aware segments from those tuples before handing the winner to [`RefinementSegment`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs)

The next blocker is to collapse that side-specific preparation under one helper boundary, so the interval-aware outer chain can prepare its winning segment without open-coding the `left/right` branch and tuple assembly inline. That keeps the early shell-edge refinement path more type-owned without adding another copied probe tier or another fallback.
...
This turn carried the early probe-entry cleanup through the remaining execution boundary: the winning interval-aware segment now owns score-based creation, stronger-segment choice, its own local-window test, and the stronger-half chase instead of splitting those steps across free helpers and the outer carrier.

The next bounded step is to remove the remaining interval-aware `left/right` split and side-specific tuple assembly from `PreparedOuterProbeChain`. If that side preparation is collapsed under one helper boundary, the unsupported-edge shell-boundary path gets a cleaner Rust-owned interval-aware refinement entry without adding another fallback tier.
