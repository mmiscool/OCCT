...
Keep narrowing the remaining shell-local OCCT bbox fallback in `offset_shell_bbox()`, but stay on the shell-boundary Rust path. The next bounded Rust-first cut is to keep the typed midpoint stage, typed outer stage, typed interval-aware side layouts, and the typed stage-result carrier in place while collapsing the remaining inline midpoint-stage -> outer-stage chaining inside the early probe stage chain.
...
- [`shell_boundary_shape_bbox()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) remains a mixed Rust/public shell-boundary union:
  ...
  - [`EarlyProbeStageChain`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now owns midpoint-stage to outer-stage progression through `stage_samples_or_refinement(...)`, while the typed [`EarlyProbeStageSamplesOrRefinement`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) carrier now keeps the interval-aware tail inside that same Rust-owned result boundary before dispatch; the top-level early probe entry now hands the chain a concrete `[NormalizedEdgeSample; 3]` source boundary directly instead of raw `start` / `midpoint` / `end` refs, and the old `EarlyProbeRefinementStages`, `EarlyProbeStageSequence`, one-use local `stage_samples!` macro, old `EarlyProbeStageOutcome` layer, nested `and_then(...)` / `finish(...)` closure chain, the `PreparedIntervalAwareRefinementSideLayouts::{needs_refinement_from_stages, needs_refinement_from_stage_progress, needs_refinement_from_stage_samples_or_result}` bounces, the old pair-level staged-progress handoff, the old pair-level `Result<Option<bool>, bool>` wrapper, the old midpoint-stage to outer-stage `and_then(...).transpose().map(Option::flatten)` bounce, the old pair-level duplicated `Ok(samples)` / `Err(result)` carry, the old pair-level `stage_samples_or_result()` bounce, the old pair-level `stage_samples_or_refinement(...)` bounce, the old pair-level final `match stage_samples { Ok(...), Err(...) }` unwrap, the old top-level `map_or_else(...)` bridge, the old interval-aware `Result<[NormalizedEdgeSample; 7], Option<bool>>` unwrap, the one-use `needs_refinement_or_continue(...)` stage helper, the one-use `needs_refinement_or_continue_with_stage(...)` / `needs_refinement_or_continue_with_stage_and_tail(...)` bridges, the old inline midpoint-stage -> outer-stage -> interval-aware continuation inside `EarlyProbeStageChain::needs_refinement()`, the old `EarlyProbeStageChain::needs_refinement()` forwarder itself, the old top-level tail-field plumbing from the early probe entry, and the old `EarlyProbeStagePair` / `EarlyProbeOuterStageTail` split are all gone
- The interval-aware refinement handoff remains typed and Rust-owned:
  ...
  - [`PreparedIntervalAwareRefinementSideLayouts`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now owns stronger coarse-side choice, winning outer-vs-inner segment selection, and the final `segment.needs_refinement(...)` dispatch directly on a concrete 7-sample boundary through `needs_refinement(...)`; the old `needs_refinement_from_stage_samples_or_refinement(...)` result-wrapper bounce and the old interval-aware terminal stage-result unwrap are gone
  - [`EarlyProbeStageChain`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now owns midpoint-stage to outer-stage progression through `stage_samples_or_refinement(...)`, and the typed [`EarlyProbeStageSamplesOrRefinement`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) carrier now owns the final stage-result -> interval-aware dispatch plus the tail state needed for that dispatch, so the old pair-level `needs_refinement()` forwarder, the old pair-level `stage_samples_or_refinement(...)` bounce, the old `continue_with_stage(...)` bounce, the old top-level `sampled_edge_interval_needs_probe_refinement()` / `map_or_else(...)` handoff, the old inline midpoint-stage -> outer-stage -> interval-aware continuation inside `needs_refinement(...)`, the old top-level tail-field plumbing, and the old `EarlyProbeStagePair` / `EarlyProbeOuterStageTail` split are all gone
...
- the midpoint-stage and outer-stage request/sample mapping now live in the same typed stage layout
  ...
  - [`EarlyProbeStageChain`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now already holds the typed midpoint stage, typed outer stage, and typed interval-aware tail, and the top-level early probe entry now hands it a concrete `[NormalizedEdgeSample; 3]` source boundary directly
  ...
  - but the early probe chain still has one smaller structural seam: [`EarlyProbeStageChain::stage_samples_or_refinement()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now builds the typed carrier with its interval-aware tail already captured, yet it still spells midpoint-stage -> outer-stage sequencing as a one-use inline `and_then(...)` chain before returning that carrier

The next blocker is to keep the typed midpoint stage, typed outer stage, typed interval-aware tail, and typed stage-result carrier, but collapse the remaining inline midpoint-stage -> outer-stage `and_then(...)` chain inside `EarlyProbeStageChain::stage_samples_or_refinement()` so that stage progression meets the stage-result carrier on a smaller Rust-owned boundary directly.
...
This turn removed the remaining top-level tail-field plumbing from the early probe entry and moved that tail state onto the typed `EarlyProbeStageSamplesOrRefinement` carrier itself. That leaves one smaller seam in this slice: `EarlyProbeStageChain::stage_samples_or_refinement()` still carries midpoint-stage into outer-stage through a one-use inline `and_then(...)` chain before it can produce that carrier.
