...
Keep narrowing the remaining M49 raw bounds fallback beyond swept faces and box planes.

`face_uv_bounds_occt(shape)` still seeds constructor-owned cylinder/cone/sphere/torus analytic faces, caps, imported faces, unsupported faces, and metadata-free faces. The next useful cut is to extend the validated analytic inventory path to another primitive constructor while preserving the raw bounds path for unsupported or ambiguous cases.

## Next Bounded Cut

1. Read `portingMilestones.md` and `nextStep.md` before editing.
2. Extend constructor-owned analytic metadata to the next exercised primitive family, preferably `make_cylinder()` side and cap faces.
3. Derive cylinder and cap face geometry seeds from Rust construction params, propagate the inventory through topology loading, and attach metadata only after normalized sample/orientation validation against each generated face.
...
- Keep direct swept, multi-face swept, box-plane analytic metadata, public payload, swept BRep solid, swept-offset metadata, multi-source swept offset, offset-solid volume, source-guard, and full-suite regressions green.
