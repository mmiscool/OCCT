...
Keep narrowing the remaining shell-local OCCT bbox fallback in `offset_shell_bbox()`, but stay on the shell-boundary Rust path. The next bounded Rust-first cut is to keep the typed two-stage early probe path and the typed interval-aware tail in place while collapsing the remaining duplicated per-stage `Ok(samples)` / `Err(result)` carry still written twice inside `EarlyProbeStagePair::needs_refinement()`.
...
- the midpoint-stage and outer-stage request/sample mapping now live in the same typed stage layout
  ...
  - but the early probe tail still has one smaller structural seam: [`EarlyProbeStagePair::needs_refinement()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) no longer uses the nested midpoint-stage -> outer-stage -> interval-aware continuation chain, yet it still repeats the same per-stage `Ok(samples)` / `Err(result)` carry twice inline even though both stages already share the same typed stage boundary

The next blocker is to keep the typed stage-pair path and the typed interval-aware tail, but collapse that remaining duplicated per-stage `Ok(samples)` / `Err(result)` carry so midpoint-stage and outer-stage progression sit on one smaller Rust-owned boundary without reintroducing the old wrappers.
...
This turn removed the one-use `needs_refinement_or_continue(...)` stage helper and flattened `EarlyProbeStagePair::needs_refinement()` onto direct sequential midpoint-stage -> outer-stage -> interval-aware dispatch. That leaves one smaller seam in this slice: the pair no longer nests the continuation chain, but it still repeats the same per-stage `Ok(samples)` / `Err(result)` carry twice inline.
