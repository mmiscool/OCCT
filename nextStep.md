...
Keep narrowing the remaining shell-local OCCT bbox fallback in `offset_shell_bbox()`, but stay on the shell-boundary Rust path. The next bounded Rust-first cut is to keep the typed two-stage early probe path and the typed interval-aware tail in place while collapsing the remaining nested midpoint-stage -> outer-stage -> tail continuation chain inside `EarlyProbeStagePair::needs_refinement()` behind one smaller Rust-owned boundary.
...
- the midpoint-stage and outer-stage request/sample mapping now live in the same typed stage layout
  ...
  - but the early probe tail still has one smaller structural seam: [`EarlyProbeStagePair::needs_refinement()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now owns the full two-stage path, yet it still spells midpoint-stage -> outer-stage -> interval-aware dispatch as one nested continuation chain inline even though both the stage boundary and the interval-aware tail are already typed

The next blocker is to keep the typed stage-pair path and the typed interval-aware tail, but collapse that remaining nested continuation chain so midpoint-stage, outer-stage, and interval-aware dispatch sit on one smaller Rust-owned boundary without reintroducing the old wrappers.
...
This turn removed the one-use `needs_refinement_or_continue_with_stage(...)` and `needs_refinement_or_continue_with_stage_and_tail(...)` helpers, and moved the top-level early probe entry onto a direct `[NormalizedEdgeSample; 3]` handoff into `EarlyProbeStagePair`. That leaves one smaller seam in this slice: the pair now owns the full two-stage path, but it still expresses midpoint-stage -> outer-stage -> interval-aware dispatch as one nested continuation chain inline.
