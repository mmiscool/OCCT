...
Keep narrowing the remaining shell-local OCCT bbox fallback in `offset_shell_bbox()`, but stay on the shell-boundary Rust path. The next bounded Rust-first cut is to collapse the remaining final `ControlFlow::Continue(samples)` vs `ControlFlow::Break(result)` unwrap now sitting inside `PreparedIntervalAwareRefinementSideLayouts::needs_refinement_from_stages(...)`, so midpoint-stage, outer-stage, and interval-aware dispatch stay on one smaller Rust-owned path without another handwritten final stage-progress match.
...
- [`shell_boundary_shape_bbox()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) remains a mixed Rust/public shell-boundary union:
  ...
  - [`EarlyProbeRefinementStages`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now keeps midpoint-stage -> outer-stage -> interval-aware progression on explicit typed stage-result values from [`EarlyProbeStageLayout`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs), so the old duplicated handwritten `ControlFlow::Continue(samples)` vs `ControlFlow::Break(result)` carry, the one-use local `stage_samples!` macro, the old `EarlyProbeStageOutcome` layer, and the nested `and_then(...)` / `finish(...)` closure chain are gone
  - [`EarlyProbeRefinementStages::needs_refinement()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now owns the top-level early-stage -> interval-aware dispatch directly, and midpoint-stage -> outer-stage carry now lives on the smaller typed `EarlyProbeStageLayout::continue_stage_progress()` boundary instead of being handwritten twice inside the interval-aware helper
...
- the midpoint-stage and outer-stage request/sample mapping now live in the same typed stage layout
  ...
  - but [`PreparedIntervalAwareRefinementSideLayouts::needs_refinement_from_stages(...)`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) still unwraps the final typed `ControlFlow::Continue(samples)` vs `ControlFlow::Break(result)` by hand once before interval-aware segment selection

The next blocker is to keep that typed stage-progress path but collapse the remaining final `ControlFlow` unwrap inside `needs_refinement_from_stages(...)` into one smaller boundary, so midpoint-stage -> outer-stage progression and interval-aware dispatch stay in the same Rust-owned path without another handwritten final stage-progress match.
...
This turn moved midpoint-stage -> outer-stage progression onto the smaller typed `continue_stage_progress(...)` boundary, so the interval-aware helper no longer spells the same direct `ControlFlow` carry twice. That leaves the next real seam smaller again: the early probe path now stays on one Rust-owned boundary, but that boundary still does one final handwritten `ControlFlow` unwrap before interval-aware segment selection.
