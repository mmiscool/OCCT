...
- The interval-aware refinement handoff remains typed and Rust-owned:
  ...
  - [`EarlyProbeStageChain`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now owns the top-level early probe entry through `needs_refinement(...)`, while typed [`EarlyProbeStageResult`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) values now own both midpoint-stage -> outer-stage continuation and final interval-aware dispatch. That leaves no extra top-level carrier between the early stage chain and the interval-aware tail.
