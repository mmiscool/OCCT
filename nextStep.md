...
Keep narrowing the remaining shell-local OCCT bbox fallback in `offset_shell_bbox()`, but stay on the shell-boundary Rust path. The next bounded Rust-first cut is to collapse the now one-use midpoint-stage -> outer-stage continuation bounce in `EarlyProbeStageLayout::continue_stage_progress()`, so the typed early-stage runner carries staged progression and interval-aware dispatch under one smaller Rust-owned boundary instead of chaining through a second continuation helper.
...
- [`shell_boundary_shape_bbox()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) remains a mixed Rust/public shell-boundary union:
  ...
  - midpoint-stage -> outer-stage chaining now lives on [`EarlyProbeStageLayout::continue_stage_progress()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs), so the one-use `outer_stage_progress()` bounce and the last manual `ControlFlow::Continue/Break` unwrap in [`EarlyProbeRefinementStages::needs_refinement()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) are both gone
...
- the midpoint-stage and outer-stage request/sample mapping now live in the same typed stage layout
  ...
  - [`EarlyProbeRefinementStages`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now owns both midpoint-stage and outer-stage source materialization, and it now forwards staged outer progress into the typed interval-aware tail instead of manually unwrapping `ControlFlow`
  ...
  - [`PreparedIntervalAwareRefinementSideLayouts`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now already owns the outer-stage `ControlFlow` unwrap, `None => false`, winning-segment selection, and terminal `segment.needs_refinement(...)` dispatch together
  ...
  - but the staged early-probe path still bounces through two one-use continuation helpers: [`EarlyProbeStageLayout::continue_stage_progress()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) and [`PreparedIntervalAwareRefinementSideLayouts::needs_refinement_from_stage_progress()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs)

The next blocker is to keep that typed `ControlFlow` progress path but stop chaining those two one-use continuation helpers, so midpoint-stage -> outer-stage progression and interval-aware dispatch both live under one smaller Rust-owned runner boundary.
...
This turn moved the final outer-stage `ControlFlow` unwrap onto the typed interval-aware side-layout boundary: [`PreparedIntervalAwareRefinementSideLayouts::needs_refinement_from_stage_progress()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now accepts staged outer progress directly, and `EarlyProbeRefinementStages::needs_refinement()` no longer spells out `Continue(samples)` vs `Break(result)` itself. That leaves the next real seam smaller again: staged progression is typed end to end, but it still passes through `continue_stage_progress()` and then `needs_refinement_from_stage_progress()` as two one-use continuation helpers.
