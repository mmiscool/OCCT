...
Keep narrowing the remaining shell-local OCCT bbox fallback in `offset_shell_bbox()`, but stay on the shell-boundary Rust path. The next bounded Rust-first cut is to collapse the repeated per-stage `Result<Option<[NormalizedEdgeSample; N]>, bool>` carry now written twice inside `EarlyProbeStagePair::needs_refinement()`, so the typed two-stage early probe progression and interval-aware dispatch can sit on one smaller Rust-owned boundary without duplicated inline stage-result translation.
...
- [`shell_boundary_shape_bbox()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) remains a mixed Rust/public shell-boundary union:
  ...
  - [`EarlyProbeStageLayout`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now owns the shared typed Rust-owned `Result<Option<[NormalizedEdgeSample; N]>, bool>` stage-result carry for both early probe stages through `stage_samples_or_result(...)`, so per-stage `Ok(samples)` / `Err(result)` translation and no-probe `None` handling stay on the stage boundary
  ...
  - the top-level early probe entry now delegates raw `start` / `midpoint` / `end` probe inputs directly into typed [`EARLY_PROBE_REFINEMENT_STAGES`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs)
  - [`EarlyProbeStagePair`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now owns midpoint-stage, outer-stage, interval-aware side layouts, and the adaptive-chase count directly, so the old `EarlyProbeRefinementStages`, `EarlyProbeStageSequence`, one-use local `stage_samples!` macro, old `EarlyProbeStageOutcome` layer, nested `and_then(...)` / `finish(...)` closure chain, the `PreparedIntervalAwareRefinementSideLayouts::{needs_refinement_from_stages, needs_refinement_from_stage_progress, needs_refinement_from_stage_samples_or_result}` bounces, the old pair-level staged-progress handoff, the old pair-level `Result<Option<bool>, bool>` wrapper, the old midpoint-stage to outer-stage `and_then(...).transpose().map(Option::flatten)` bounce, and the old pair-level `stage_samples_or_result()` bounce are all gone
...
- the midpoint-stage and outer-stage request/sample mapping now live in the same typed stage layout
  - [`EarlyProbeStageLayout`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now owns direct `probe_pair(...) -> staged sample roles -> Result<Option<[NormalizedEdgeSample; N]>, bool>` execution for each early probe stage
  ...
  - [`EarlyProbeStagePair`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now already holds midpoint-stage, outer-stage, interval-aware tail configuration, and the adaptive-chase count together once raw `start` / `midpoint` / `end` inputs arrive
  - the old `EarlyProbeStageLayout::refinement_result()`, `continue_with_stage()`, and `continue_stage_progress()` bounces are gone, and `stage_samples_or_result()` now stays only as the per-stage typed execution boundary
  - [`PreparedIntervalAwareRefinementSideLayouts`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now already owns `Err(result) => Some(result)`, `Ok(None) => None`, winning-segment selection, and terminal `segment.needs_refinement(...)` dispatch together once it receives the final 7-sample boundary
  ...
  - but [`EarlyProbeStagePair::needs_refinement()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now carries the same inline `Ok(Some(samples))` / `Ok(None)` / `Err(result)` translation twice, once for midpoint-stage and once for outer-stage

The next blocker is to keep the typed stage-pair path but collapse that repeated per-stage `Result<Option<[NormalizedEdgeSample; N]>, bool>` carry inside `EarlyProbeStagePair::needs_refinement()`, so the two-stage early probe progression and interval-aware dispatch can stay on one smaller Rust-owned boundary without duplicated inline stage-result translation.
...
This turn first removed the one-use `PreparedIntervalAwareRefinementSideLayouts::needs_refinement_from_stage_samples_or_result()` wrapper, then folded the pair-level `stage_samples_or_result()` bounce into `EarlyProbeStagePair::needs_refinement()`. That leaves the next real seam smaller again: the same stage-result translation is now written twice inline inside `EarlyProbeStagePair::needs_refinement()`, once for midpoint-stage and once for outer-stage.
