...
Keep narrowing the remaining shell-local OCCT bbox fallback in `offset_shell_bbox()`, but stay on the shell-boundary Rust path. The next bounded Rust-first cut is to collapse the two remaining one-use continuation helpers around the shared two-stage early probe runner, `EarlyProbeStageLayout::continue_stage_progress()` and `PreparedIntervalAwareRefinementSideLayouts::needs_refinement_from_stage_progress()`, so midpoint-stage, outer-stage, and interval-aware dispatch sit on one smaller Rust-owned runner boundary without an extra cross-helper `ControlFlow` carry.
...
- [`shell_boundary_shape_bbox()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) remains a mixed Rust/public shell-boundary union:
  ...
  - [`EarlyProbeRefinementStages::needs_refinement()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now forwards midpoint-stage -> outer-stage stage progress straight into [`PreparedIntervalAwareRefinementSideLayouts::needs_refinement_from_stage_progress()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs), so the final interval-aware `ControlFlow::Continue(samples)` vs `ControlFlow::Break(result)` unwrap is gone from the runner too
...
- the midpoint-stage and outer-stage request/sample mapping now live in the same typed stage layout
  ...
  - but the staged early-probe path still bounces through two one-use continuation helpers, [`EarlyProbeStageLayout::continue_stage_progress()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) and [`PreparedIntervalAwareRefinementSideLayouts::needs_refinement_from_stage_progress()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs), between midpoint-stage execution and interval-aware dispatch

The next blocker is to keep that typed `ControlFlow` progress path but collapse those two one-use continuation helpers into one smaller runner-owned boundary, so midpoint-stage -> outer-stage progression and interval-aware dispatch stay in the same Rust-owned path without a leftover cross-helper carry.
...
This turn moved the final staged `ControlFlow` unwrap onto [`PreparedIntervalAwareRefinementSideLayouts`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs), so midpoint-stage, outer-stage, and interval-aware dispatch are all now typed through the same early-probe path. That leaves the next real seam smaller again: the behavior is already shared, but it still crosses two one-use continuation helpers before reaching the interval-aware segment chooser.
