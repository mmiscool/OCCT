...
Keep narrowing the remaining shell-local OCCT bbox fallback in `offset_shell_bbox()`, but stay on the shell-boundary Rust path. The next bounded Rust-first cut is to keep the typed two-stage early probe path in place while collapsing the remaining midpoint-stage to outer-stage `and_then(...)` chain inside `EarlyProbeStagePair::needs_refinement()`.
...
- [`shell_boundary_shape_bbox()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) remains a mixed Rust/public shell-boundary union:
  ...
  - [`EarlyProbeStagePair`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now owns midpoint-stage, outer-stage, interval-aware side layouts, and the adaptive-chase count directly through `needs_refinement()`, and midpoint-stage to outer-stage sequencing now stays inline there on the shared typed `Result<[NormalizedEdgeSample; 7], Option<bool>>` stage-result boundary, so the old `EarlyProbeRefinementStages`, `EarlyProbeStageSequence`, one-use local `stage_samples!` macro, old `EarlyProbeStageOutcome` layer, nested `and_then(...)` / `finish(...)` closure chain, the `PreparedIntervalAwareRefinementSideLayouts::{needs_refinement_from_stages, needs_refinement_from_stage_progress, needs_refinement_from_stage_samples_or_result}` bounces, the old pair-level staged-progress handoff, the old pair-level `Result<Option<bool>, bool>` wrapper, the old midpoint-stage to outer-stage `and_then(...).transpose().map(Option::flatten)` bounce, the old inline midpoint-stage to outer-stage `and_then(...)` bridge, the old pair-level `stage_samples_or_result()` bounce, the old duplicated per-stage `Ok(samples)` / `Err(result)` unwrap, the old pair-level `stage_samples_or_refinement(...)` bounce, and the old top-level `map_or_else(...)` bridge are all gone
- The interval-aware refinement handoff remains typed and Rust-owned:
  ...
  - [`PreparedIntervalAwareRefinementSideLayouts`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now owns the terminal stage-result unwrap, stronger coarse-side choice, winning outer-vs-inner segment selection, and the final `segment.needs_refinement(...)` dispatch directly through `needs_refinement(...)`, so the old `needs_refinement_from_stage_samples_or_refinement(...)` result-wrapper bounce is gone
...
- the midpoint-stage and outer-stage request/sample mapping now live in the same typed stage layout
  ...
  - but [`EarlyProbeStagePair::needs_refinement()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) still carries one smaller internal seam: it now sequences midpoint-stage and outer-stage on that shared pair-level stage result, but still does it through a one-use inline `and_then(...)` chain before interval-aware dispatch

The next blocker is to keep the typed stage-pair path but collapse that remaining midpoint-stage to outer-stage `and_then(...)` chain, so midpoint-stage, outer-stage, and interval-aware dispatch can stay on one smaller Rust-owned boundary inside `EarlyProbeStagePair::needs_refinement()`.
...
This turn removed the one-use pair-level `stage_samples_or_refinement(...)` helper from `EarlyProbeStagePair::needs_refinement()`, so midpoint-stage to outer-stage sequencing now stays inline on the pair boundary. The remaining structural seam in this slice is now that one-use inline `and_then(...)` chain before interval-aware dispatch.
