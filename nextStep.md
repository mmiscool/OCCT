...
- [`shell_boundary_shape_bbox()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) is now a mixed per-edge union:
  ...
  - if those fixed outer probes still decline, the same tier now also scores the left and right side triples using the same bbox-expansion, tangent-turn, axis-shoulder, and chord-bend signals, then scores the inner and outer half of the stronger side, then scores the start-shoulder and end-shoulder segment of that chosen half, then scores the start-endpoint and end-endpoint subsegment of that chosen shoulder segment, then scores the start-terminal and end-terminal subsegment of that chosen endpoint segment, then scores the start-terminal-endpoint and end-terminal-endpoint subsegment of that chosen terminal segment, then biases the actual refinement probe toward the stronger endpoint side of that chosen terminal-endpoint segment, then chooses between the inner and outer half of that stronger endpoint side, then chooses between the inner and outer sub-half of that chosen stronger half, then chooses between the inner and outer half of that chosen stronger sub-half, then chooses between the inner and outer half of that chosen stronger half of that chosen stronger sub-half with that same signal mix before deciding whether to subdivide
...
The remaining blocker is shell edges whose decisive bbox extrema still evade the new score-driven side, half, shoulder-end, endpoint, terminal, and terminal-endpoint choice because the suspicious endpoint side inside the chosen terminal-endpoint subsegment is now known, the suspicious inner-vs-outer half of that stronger endpoint side is now known, the suspicious inner-vs-outer sub-half of that stronger half is now known, the suspicious inner-vs-outer half of that stronger sub-half is now known, and the suspicious inner-vs-outer half of that stronger half of that stronger sub-half is now known, but the current probe inside that newly-chosen segment still lands at a fixed midpoint. Even after the midpoint axis-shoulder refinement trigger, the quarter-point probe-driven refinement prepass, the outer-probe refinement pass, the stronger-side, stronger-half, stronger-shoulder, stronger-endpoint, stronger-terminal, stronger-terminal-endpoint, stronger-terminal-endpoint-side biased probe, stronger-terminal-endpoint-side-half chooser, stronger-terminal-endpoint-side-half sub-half chooser, stronger-terminal-endpoint-side-half-sub-half chooser, stronger-terminal-endpoint-side-half-sub-half-half chooser, bracketed tangent-root polish, the near-flat tangent-dip search, the local axis-position extremum search, and the broader run-based seeded axis-position search, those shells still skip straight to the later mesh/summary candidates and eventually the raw shell-local OCCT bbox.
...
- if those fixed outer probes still decline, the refinement gate now also scores the left and right side triples with the same existing refinement signals, then chooses between the inner and outer half of the stronger side, then between the start-shoulder and end-shoulder segment of that chosen half, then between the start-endpoint and end-endpoint subsegment of that chosen shoulder segment, then between the start-terminal and end-terminal subsegment of that chosen endpoint segment, then between the start-terminal-endpoint and end-terminal-endpoint subsegment of that chosen terminal segment, then biases the actual refinement probe toward the stronger endpoint side inside that chosen terminal-endpoint subsegment, then chooses between the inner and outer half of that stronger endpoint side, then chooses between the inner and outer sub-half of that stronger half, then chooses between the inner and outer half of that stronger sub-half, then chooses between the inner and outer half of that stronger half of that stronger sub-half before reusing the existing refinement test on that more narrowly placed probe, so one-sided intervals can still trigger recursive public sampling before the later tangent and position solvers run

The next step is to make that shell-local Rust boundary path cover more real offset shells by refining where the score-driven extra probe lands inside the already-chosen stronger half of the already-chosen stronger half of the already-chosen stronger sub-half of the stronger endpoint side of the terminal-endpoint subsegment. The next bounded cut is replacing that segment-local fixed midpoint with a score-driven inner-vs-outer placement inside that stronger half of that stronger half of that stronger sub-half from the same signal mix, not widening fallback elsewhere.
