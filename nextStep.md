...
Keep narrowing the remaining shell-local OCCT bbox fallback in `offset_shell_bbox()`, but stay on the shell-boundary Rust path. The next bounded Rust-first cut is to collapse the remaining final `ControlFlow::Continue(samples)` vs `ControlFlow::Break(result)` unwrap between the shared two-stage early probe runner and interval-aware dispatch, so the typed early-stage path keeps midpoint-stage, outer-stage, and interval-aware handoff on one smaller Rust-owned boundary instead of bouncing that last carry back into `EarlyProbeRefinementStages::needs_refinement()`.
...
- [`shell_boundary_shape_bbox()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) remains a mixed Rust/public shell-boundary union:
  ...
  - [`EarlyProbeRefinementStages::needs_refinement()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now carries midpoint-stage -> outer-stage progression through shared typed [`EarlyProbeStageLayout::continue_stage_progress()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) execution, so the duplicated midpoint-stage vs outer-stage `ControlFlow::Continue/Break` carry is gone and only the final interval-aware handoff still unwraps stage progress
...
- the midpoint-stage and outer-stage request/sample mapping now live in the same typed stage layout
  ...
  - but the staged early-probe path still unwraps the final `ControlFlow::Continue(samples)` vs `ControlFlow::Break(result)` by hand once inside [`EarlyProbeRefinementStages::needs_refinement()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) before interval-aware dispatch

The next blocker is to keep that typed `ControlFlow` progress path but move the remaining final carry onto the interval-aware side-layout boundary, so midpoint-stage -> outer-stage progression and interval-aware dispatch still live under the same Rust-owned runner boundary without a leftover runner-local `Continue/Break` unwrap.
...
This turn shared the duplicated midpoint-stage -> outer-stage stage-progress carry behind [`EarlyProbeStageLayout::continue_stage_progress()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs), so `EarlyProbeRefinementStages::needs_refinement()` no longer spells out the same `ControlFlow::Continue(samples)` vs `ControlFlow::Break(result)` handoff twice. That leaves the next real seam smaller again: staged progression is typed through midpoint-stage and outer-stage already, but the runner still unwraps the final `ControlFlow` once before interval-aware dispatch instead of letting the typed interval-aware boundary own that last carry too.
