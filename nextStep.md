...
Keep narrowing the remaining shell-local OCCT bbox fallback in `offset_shell_bbox()`, but stay on the shell-boundary Rust path. The next bounded Rust-first cut is to collapse the remaining midpoint-vs-outer `source_position()` remap in the early shell-edge refinement path, so typed source-slot enums stop each spelling out the same ordinal walk into the shared `EarlyProbeSourcePosition` boundary.
...
- but the typed source-slot enums still duplicate the stage-local slot-to-position remap one level higher: midpoint-stage and outer-stage each still keep their own `source_position()` match that translates typed slots into the shared `EarlyProbeSourcePosition`

The next blocker is to collapse that remaining midpoint-vs-outer `source_position()` remap into a smaller shared typed boundary, so the stage-specific source-slot enums stop each carrying their own ordinal walk into `EarlyProbeSourcePosition`.
...
This turn finished the shared typed source-position cut: midpoint-stage and outer-stage source-slot enums now end at the same `EarlyProbeSourcePosition` boundary, and the duplicated raw `source_index()` remaps are gone. That leaves the next real seam one level smaller again: the stage-specific source-slot enums still duplicate the slot-to-position remap in their own `source_position()` matches.
