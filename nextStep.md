...
Keep narrowing the remaining shell-local OCCT bbox fallback in `offset_shell_bbox()`, but stay on the shell-boundary Rust path. The next bounded Rust-first cut is to keep the typed two-stage early probe path and the typed interval-aware tail in place while collapsing the remaining inline midpoint-stage -> outer-stage `and_then(...)` chain still sitting inside `EarlyProbeStagePair::needs_refinement()`.
...
- the midpoint-stage and outer-stage request/sample mapping now live in the same typed stage layout
  ...
  - but the early probe tail still has one smaller structural seam: [`EarlyProbeStagePair::needs_refinement()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) no longer repeats the per-stage `Ok(samples)` / `Err(result)` carry twice inline, yet it still spells midpoint-stage -> outer-stage progression as one inline `and_then(...)` chain before the interval-aware tail

The next blocker is to keep the typed stage-pair path and the typed interval-aware tail, but collapse that remaining inline midpoint-stage -> outer-stage `and_then(...)` chain so stage-to-stage progression sits on one smaller Rust-owned boundary without reintroducing the old wrappers.
...
This turn kept `EarlyProbeStagePair::needs_refinement()` on the direct sequential midpoint-stage -> outer-stage -> interval-aware path, but collapsed the duplicated per-stage `Ok(samples)` / `Err(result)` carry into one shared `and_then(...)` chain on the typed stage boundary. That leaves one smaller seam in this slice: the pair still spells the midpoint-stage -> outer-stage progression inline instead of hiding that stage-to-stage carry behind a smaller Rust-owned boundary.
