...
Keep narrowing the remaining shell-local OCCT bbox fallback in `offset_shell_bbox()`, but stay on the shell-boundary Rust path. The next bounded Rust-first cut is to collapse the remaining duplicated `Ok(samples) / Err(result)` split in the early shell-edge refinement path, so `EarlyProbeRefinementStages` keeps midpoint-stage -> outer-stage -> interval-aware progression on one explicit typed runner boundary without repeating the same stage-result unwrap twice in `needs_refinement()`.
...
- the midpoint-stage and outer-stage request/sample mapping now live in the same typed stage layout
  ...
  - but `EarlyProbeRefinementStages::needs_refinement()` still unwraps the shared stage-result path twice by hand, once after midpoint-stage and once after outer-stage, through the same `Ok(samples) => samples` / `Err(result) => return Some(result)` split, even though per-stage execution already lives on `EarlyProbeStageLayout`, `EarlyProbeStageOutcome` now owns its own result translation, and the runner already owns the typed interval-aware tail

The next blocker is to collapse that remaining duplicated stage-result unwrap, so the early stage runner stays on one Rust-owned midpoint-stage -> outer-stage -> interval-aware execution path under the explicit typed stage boundary without repeating the same `Ok(samples) => samples` / `Err(result) => return Some(result)` split per stage.
...
This turn finished the direct `EarlyProbeStageOutcome` variant unwrap in the runner: midpoint-stage and outer-stage still progress through explicit `EarlyProbeStageOutcome` values, but the outcome type now owns its own `Complete` vs `Samples` translation through `refinement_result()`. That leaves the next real seam smaller again: `EarlyProbeRefinementStages::needs_refinement()` still repeats the same `Ok(samples) => samples` / `Err(result) => return Some(result)` split twice, once per early stage, even though the stage execution boundary and the typed interval-aware tail are already in place.
