...
Keep narrowing the remaining M49 raw bounds fallback beyond swept faces, box planes, cylinder side/cap faces, and cone side/cap faces.

`face_uv_bounds_occt(shape)` still seeds constructor-owned sphere/torus analytic faces, imported faces, unsupported faces, and metadata-free faces. The next useful cut is to extend the validated analytic inventory path to another primitive constructor while preserving the raw bounds path for unsupported or ambiguous cases.

## Next Bounded Cut

1. Read `portingMilestones.md` and `nextStep.md` before editing.
2. Extend constructor-owned analytic metadata to the next exercised primitive family, preferably `make_sphere()` faces.
3. Derive sphere face geometry seeds from Rust construction params, propagate the inventory through topology loading, and attach metadata only after normalized sample/orientation validation against each generated face.
...
- Keep direct swept, multi-face swept, box-plane analytic metadata, cylinder analytic metadata, cone analytic metadata, public payload, swept BRep solid, swept-offset metadata, multi-source swept offset, offset-solid volume, source-guard, and full-suite regressions green.
