...
Keep narrowing the remaining shell-local OCCT bbox fallback in `offset_shell_bbox()`, but stay on the shell-boundary Rust path. The next bounded Rust-first cut is to collapse the remaining duplicated midpoint-stage vs outer-stage `ControlFlow` carry inside `EarlyProbeRefinementStages::needs_refinement()`, so the typed early-stage runner keeps the full two-stage progression on one smaller Rust-owned boundary instead of spelling the same staged `Continue/Break` handoff twice.
...
- the midpoint-stage and outer-stage request/sample mapping now live in the same typed stage layout
  ...
  - but the staged early-probe path still spells out the same direct `ControlFlow::Continue(samples)` vs `ControlFlow::Break(result)` carry twice inside [`EarlyProbeRefinementStages::needs_refinement()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs): once after midpoint-stage and once after outer-stage

The next blocker is to keep that typed `ControlFlow` progress path but share the remaining direct stage carry, so midpoint-stage -> outer-stage progression and interval-aware dispatch still live under the same Rust-owned runner boundary without duplicating the same `Continue/Break` split twice.
...
This turn removed both remaining one-use continuation helpers from the early probe path: [`EarlyProbeStageLayout::continue_stage_progress()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) and [`PreparedIntervalAwareRefinementSideLayouts::needs_refinement_from_stage_progress()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) are gone, and `EarlyProbeRefinementStages::needs_refinement()` now carries midpoint-stage -> outer-stage -> interval-aware progression directly. That leaves the next real seam smaller again: staged progression is typed end to end, but the runner still repeats the same `ControlFlow::Continue(samples)` vs `ControlFlow::Break(result)` carry once per early stage.
