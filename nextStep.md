...
Keep narrowing the remaining shell-local OCCT bbox fallback in `offset_shell_bbox()`, but stay on the shell-boundary Rust path. The mixed-edge shell-boundary union now includes an adaptive sampled public-edge tier plus four public unsupported-edge extremum passes: bracketed axis-turning polish, a near-flat tangent-dip probe, a local axis-position extremum search, and a broader run-based seeded axis-position extremum search. The next task is refining the new score-driven endpoint probe placement inside the already-chosen shoulder segment, so one-sided shell-edge extrema can trigger denser public sampling even when the current fixed endpoint-local midpoint still misses them.
...
- [`shell_boundary_shape_bbox()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) is now a mixed per-edge union:
  ...
  - if those fixed outer probes still decline, the same tier now also scores the left and right side triples using the same bbox-expansion, tangent-turn, axis-shoulder, and chord-bend signals, then scores the inner and outer half of the stronger side, then scores the start-shoulder and end-shoulder segment of that chosen half, then scores the start-endpoint and end-endpoint subsegment of that chosen shoulder segment with that same signal mix before deciding whether to subdivide
...
The remaining blocker is shell edges whose decisive bbox extrema still evade the new score-driven side, half, shoulder-end, and endpoint choice because the suspicious endpoint subsegment is known but the current endpoint-local probe still lands at a fixed midpoint inside that segment. Even after the midpoint axis-shoulder refinement trigger, the quarter-point probe-driven refinement prepass, the outer-probe refinement pass, the stronger-side, stronger-half, stronger-shoulder, and stronger-endpoint biased probe, bracketed tangent-root polish, the near-flat tangent-dip search, the local axis-position extremum search, and the broader run-based seeded axis-position search, those shells still skip straight to the later mesh/summary candidates and eventually the raw shell-local OCCT bbox.
...
- if those fixed outer probes still decline, the refinement gate now also scores the left and right side triples with the same existing refinement signals, then chooses between the inner and outer half of the stronger side, then between the start-shoulder and end-shoulder segment of that chosen half, then between the start-endpoint and end-endpoint subsegment of that chosen shoulder segment, before reusing the existing refinement test on that endpoint subsegment, so one-sided intervals can still trigger recursive public sampling before the later tangent and position solvers run

The next step is to make that shell-local Rust boundary path cover more real offset shells by refining where the score-driven extra probe lands inside the already-chosen endpoint subsegment. The next bounded cut is biasing that endpoint-local probe away from its fixed midpoint toward the stronger endpoint inside the chosen endpoint subsegment from the same signal mix, not widening fallback elsewhere.
