...
Keep narrowing the remaining shell-local OCCT bbox fallback in `offset_shell_bbox()`, but stay on the shell-boundary Rust path. The next bounded Rust-first cut is to collapse the now one-use `EarlyProbeStageSequence::stage_progress()` bounce, so midpoint-stage, outer-stage, and interval-aware dispatch stay on one smaller Rust-owned path without a separate two-stage sequence wrapper between `EarlyProbeRefinementStages` and the per-stage layouts.
...
- [`shell_boundary_shape_bbox()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) remains a mixed Rust/public shell-boundary union:
  ...
  - [`EarlyProbeStageLayout`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now owns the shared typed Rust-owned `ControlFlow<bool, [NormalizedEdgeSample; N]>` stage-progress carry for both early probe stages through `stage_progress(...)`
  ...
  - [`EarlyProbeStageSequence`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now owns midpoint-stage -> outer-stage progression on explicit typed stage-progress values from [`EarlyProbeStageLayout`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs), so the old duplicated handwritten `ControlFlow::Continue(samples)` vs `ControlFlow::Break(result)` carry, the one-use local `stage_samples!` macro, the old `EarlyProbeStageOutcome` layer, and the nested `and_then(...)` / `finish(...)` closure chain are gone from [`EarlyProbeRefinementStages::needs_refinement()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs)
  - [`EarlyProbeRefinementStages::needs_refinement()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now owns the top-level early-stage -> interval-aware dispatch directly through [`EarlyProbeStageSequence`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs), the old `PreparedIntervalAwareRefinementSideLayouts::needs_refinement_from_stages(...)` bounce is gone, and the old `PreparedIntervalAwareRefinementSideLayouts::needs_refinement_from_stage_progress(...)` bounce is gone too
...
- the midpoint-stage and outer-stage request/sample mapping now live in the same typed stage layout
  ...
  - the stage runner now receives raw `start` / `midpoint` / `end` inputs directly, and midpoint-stage, outer-stage, and interval-aware tail configuration all already live in [`EarlyProbeRefinementStages`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) plus its typed [`EarlyProbeStageSequence`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs)
  ...
  - [`EarlyProbeStageSequence`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now owns the direct midpoint-stage -> outer-stage control flow, and [`EarlyProbeRefinementStages`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now just threads that typed two-stage result into interval-aware dispatch
  ...
  - but midpoint-stage -> outer-stage progression now lives in a one-use [`EarlyProbeStageSequence::stage_progress()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) wrapper, even though the per-stage execution boundary and the interval-aware handoff are already typed

The next blocker is to keep that typed stage-progress path but collapse the remaining one-use `EarlyProbeStageSequence` bounce, so midpoint-stage -> outer-stage progression and interval-aware dispatch stay in the same Rust-owned path without a second sequence-only boundary between the runner and the per-stage layouts.
...
This turn removed the direct midpoint-stage `ControlFlow` carry from `EarlyProbeRefinementStages::needs_refinement()` by moving midpoint-stage -> outer-stage progression onto `EarlyProbeStageSequence`. That leaves the next real seam smaller again: the handwritten runner match is gone, but the progression still sits behind a one-use typed sequence wrapper.
