...
Keep narrowing the remaining shell-local OCCT bbox fallback in `offset_shell_bbox()`, but stay on the shell-boundary Rust path. The next bounded Rust-first cut is to keep the typed two-stage early probe path in place while collapsing the remaining pair-level terminal `map_or_else(...)` bounce inside `EarlyProbeStagePair::needs_refinement()`.
...
- The interval-aware refinement handoff remains typed and Rust-owned:
  ...
  - [`EarlyProbeStagePair`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now owns midpoint-stage to outer-stage progression and interval-aware dispatch through `needs_refinement(...)`, so the old pair-level `needs_refinement()` forwarder and the old top-level `sampled_edge_interval_needs_probe_refinement()` / `map_or_else(...)` handoff are both gone
...
- the midpoint-stage and outer-stage request/sample mapping now live in the same typed stage layout
  ...
  - but [`EarlyProbeStagePair::needs_refinement()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) still carries one smaller internal seam: it now sequences midpoint-stage and outer-stage on that shared pair-level stage result, but still ends with a one-use `map_or_else(...)` terminal bounce before interval-aware dispatch

The next blocker is to keep the typed stage-pair path but collapse that remaining pair-level `map_or_else(...)` terminal bounce, so midpoint-stage, outer-stage, and interval-aware dispatch can stay on one smaller Rust-owned boundary inside `EarlyProbeStagePair::needs_refinement()`.
...
This turn removed the duplicated per-stage `match` carry from `EarlyProbeStagePair::needs_refinement()` by moving midpoint-stage to outer-stage sequencing onto a shared typed pair-level stage result, so the remaining structural seam in this slice is now the final pair-level `map_or_else(...)` bounce before interval-aware dispatch.
