...
Keep narrowing the remaining shell-local OCCT bbox fallback in `offset_shell_bbox()`, but stay on the shell-boundary Rust path. The next bounded Rust-first cut is to keep the typed two-stage early probe path and the typed interval-aware tail in place while collapsing the remaining thin pair-level bridge that still chains `midpoint_stage.continue_with_stage(...)` into `interval_aware_tail.needs_refinement_from_stage_samples_or_refinement(...)`.
...
- The interval-aware refinement handoff remains typed and Rust-owned:
  ...
  - [`PreparedIntervalAwareRefinementSideLayouts`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now owns stronger coarse-side choice, winning outer-vs-inner segment selection, and the final `segment.needs_refinement(...)` dispatch directly on a concrete 7-sample boundary through `needs_refinement(...)`, while [`EarlyProbeIntervalAwareTail`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now owns the final stage-result unwrap before that dispatch; the old `needs_refinement_from_stage_samples_or_refinement(...)` result-wrapper bounce and the old interval-aware terminal stage-result unwrap are gone
...
- the midpoint-stage and outer-stage request/sample mapping now live in the same typed stage layout
  ...
  - [`EarlyProbeStagePair`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now already holds midpoint-stage, outer-stage, and the typed [`EarlyProbeIntervalAwareTail`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs), and the top-level early probe entry now hands it a concrete `[NormalizedEdgeSample; 3]` source boundary directly
  ...
  - but the early probe pair still has one smaller structural seam: [`EarlyProbeStagePair::needs_refinement()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) no longer spells midpoint-stage -> outer-stage progression as an inline `and_then(...)` chain and no longer performs the final pair-level `match stage_samples { Ok(...), Err(...) }` unwrap, yet it is still just a thin bridge from `midpoint_stage.continue_with_stage(...)` into `interval_aware_tail.needs_refinement_from_stage_samples_or_refinement(...)`

The next blocker is to keep the typed stage-pair path and the typed interval-aware tail, but collapse that remaining pair-level bridge so stage-to-stage progression and the final interval-aware dispatch sit on one smaller Rust-owned boundary without reintroducing the old wrappers.
...
This turn kept `EarlyProbeStagePair::needs_refinement()` on the direct sequential midpoint-stage -> outer-stage -> interval-aware path, but moved the final two-stage result unwrap onto the typed `EarlyProbeIntervalAwareTail`. That leaves one smaller seam in this slice: the pair is now just a thin bridge from `continue_with_stage(...)` into the tail’s `needs_refinement_from_stage_samples_or_refinement(...)` helper.
