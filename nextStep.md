...
Remove or strictly narrow the next public payload fallback family: `rust/lean_occt/src/lib.rs::face_offset_basis_curve_geometry()` and the public offset-basis curve payload wrappers still call direct `face_offset_basis_curve_*_occt()` helpers when `ported_offset_surface()` returns `None`.

## Next Bounded Cut

1. Start with the offset-basis curve family in `rust/lean_occt/src/lib.rs`: `face_offset_basis_curve_geometry()`, `face_offset_basis_curve_line_payload()`, `face_offset_basis_curve_circle_payload()`, and `face_offset_basis_curve_ellipse_payload()`.
2. Reuse `ported_offset_face_surface_payload()` so supported swept Offset faces require a Rust `PortedOffsetSurface` descriptor with a swept `PortedCurve` basis before public offset-basis curve geometry or curve payload APIs can succeed.
3. Preserve the explicit `face_offset_basis_curve_*_occt()` APIs as opt-in parity/oracle APIs, and keep test oracle usage explicit.
4. Keep mismatched curve payload requests as explicit Rust errors, and keep `public_offset_basis_queries_match_occt` proving descriptor routing before OCCT oracle comparison.
