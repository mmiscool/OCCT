...
Keep narrowing the remaining shell-local OCCT bbox fallback in `offset_shell_bbox()`, but stay on the shell-boundary Rust path. The next bounded Rust-first cut is to keep the typed midpoint stage, typed outer stage, and typed interval-aware side layouts in place while collapsing the remaining inline midpoint-stage -> outer-stage -> interval-aware continuation inside `EarlyProbeStageChain::needs_refinement()`.
...
- The interval-aware refinement handoff remains typed and Rust-owned:
  ...
  - [`EarlyProbeStageChain`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now owns midpoint-stage to outer-stage progression and interval-aware dispatch through `needs_refinement(...)`, so the old pair-level `needs_refinement()` forwarder, the old pair-level `stage_samples_or_refinement(...)` bounce, the old `continue_with_stage(...)` bounce, the old top-level `sampled_edge_interval_needs_probe_refinement()` / `map_or_else(...)` handoff, and the old `EarlyProbeStagePair` / `EarlyProbeOuterStageTail` split are all gone
...
- the midpoint-stage and outer-stage request/sample mapping now live in the same typed stage layout
  ...
  - but the early probe stage chain still has one smaller structural seam: [`EarlyProbeStageChain::needs_refinement()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) still spells the midpoint-stage -> outer-stage -> interval-aware progression inline through one `and_then(...).map_or_else(...)` chain, so the remaining cleanup is collapsing that inline continuation onto a smaller typed boundary instead of reintroducing the old deleted wrappers

The next blocker is to keep the typed midpoint stage, typed outer stage, and typed interval-aware side layouts, but collapse the remaining inline midpoint-stage -> outer-stage -> interval-aware continuation inside `EarlyProbeStageChain::needs_refinement()` so that progression sits on one smaller Rust-owned boundary without reintroducing the old deleted wrappers.
...
This turn collapsed the thin `EarlyProbeStagePair` / `EarlyProbeOuterStageTail` split into one typed `EarlyProbeStageChain`. That leaves one smaller seam in this slice: `EarlyProbeStageChain::needs_refinement()` still spells the midpoint-stage -> outer-stage -> interval-aware continuation inline.
