...
Keep narrowing the remaining shell-local OCCT bbox fallback in `offset_shell_bbox()`, but stay on the shell-boundary Rust path. The next bounded Rust-first cut is to keep the typed midpoint stage, typed outer stage, and typed interval-aware side layouts in place while collapsing the remaining final stage-chain -> interval-aware handoff inside `EarlyProbeStageChain::needs_refinement()`.
...
- The interval-aware refinement handoff remains typed and Rust-owned:
  ...
  - [`EarlyProbeStageChain`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now owns midpoint-stage to outer-stage progression through `stage_samples_or_refinement(...)` and interval-aware dispatch through `needs_refinement(...)`, so the old pair-level `needs_refinement()` forwarder, the old pair-level `stage_samples_or_refinement(...)` bounce, the old `continue_with_stage(...)` bounce, the old top-level `sampled_edge_interval_needs_probe_refinement()` / `map_or_else(...)` handoff, the old inline midpoint-stage -> outer-stage -> interval-aware continuation inside `needs_refinement(...)`, and the old `EarlyProbeStagePair` / `EarlyProbeOuterStageTail` split are all gone
...
- the midpoint-stage and outer-stage request/sample mapping now live in the same typed stage layout
  ...
  - [`EarlyProbeStageChain`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now already holds the typed midpoint stage, typed outer stage, and typed interval-aware side layouts, and the top-level early probe entry now hands it a concrete `[NormalizedEdgeSample; 3]` source boundary directly
  ...
  - but the early probe stage chain still has one smaller structural seam: [`EarlyProbeStageChain::needs_refinement()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now delegates midpoint-stage -> outer-stage progression through `stage_samples_or_refinement(...)`, yet it still finishes with one `map_or_else(...)` bounce into [`PreparedIntervalAwareRefinementSideLayouts::needs_refinement()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs), so the remaining cleanup is collapsing that final stage-chain -> interval-aware handoff onto a smaller typed boundary without reintroducing the old deleted wrappers

The next blocker is to keep the typed midpoint stage, typed outer stage, and typed interval-aware side layouts, but collapse the remaining final `stage_samples_or_refinement(...).map_or_else(...)` handoff inside `EarlyProbeStageChain::needs_refinement()` so that the early stage chain and interval-aware tail meet on one smaller Rust-owned boundary without reintroducing the old deleted wrappers.
...
This turn moved midpoint-stage -> outer-stage progression onto `EarlyProbeStageChain::stage_samples_or_refinement(...)`. That leaves one smaller seam in this slice: `EarlyProbeStageChain::needs_refinement()` still finishes with the final `stage_samples_or_refinement(...).map_or_else(...)` handoff into the interval-aware side layouts.
