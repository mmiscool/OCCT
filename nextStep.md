...
Keep narrowing the remaining shell-local OCCT bbox fallback in `offset_shell_bbox()`, but stay on the shell-boundary Rust path. The next bounded Rust-first cut is to collapse the now one-use `EarlyProbeRefinementStages::stage_progress()` bounce, so midpoint-stage, outer-stage, and interval-aware dispatch stay on one smaller Rust-owned path without a separate two-stage progression helper inside the typed runner.
...
- [`shell_boundary_shape_bbox()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) remains a mixed Rust/public shell-boundary union:
  ...
  - [`EarlyProbeRefinementStages`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now owns midpoint-stage -> outer-stage progression on explicit typed stage-progress values from [`EarlyProbeStageLayout`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs), so the separate `EarlyProbeStageSequence` layer is gone along with the old duplicated handwritten `ControlFlow::Continue(samples)` vs `ControlFlow::Break(result)` carry, the one-use local `stage_samples!` macro, the old `EarlyProbeStageOutcome` layer, and the nested `and_then(...)` / `finish(...)` closure chain from [`EarlyProbeRefinementStages::needs_refinement()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs)
...
- the midpoint-stage and outer-stage request/sample mapping now live in the same typed stage layout
  ...
  - [`EarlyProbeRefinementStages`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) now owns the direct midpoint-stage -> outer-stage control flow and threads that typed two-stage result into interval-aware dispatch itself
  ...
  - but midpoint-stage -> outer-stage progression still lives behind a one-use [`EarlyProbeRefinementStages::stage_progress()`](rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs) helper, even though the per-stage execution boundary and the interval-aware handoff are already typed

The next blocker is to keep that typed stage-progress path but collapse the remaining one-use `EarlyProbeRefinementStages::stage_progress()` bounce, so midpoint-stage -> outer-stage progression and interval-aware dispatch stay in the same Rust-owned path without a second helper-only boundary inside the runner.
...
This turn removed the separate `EarlyProbeStageSequence` layer by folding midpoint-stage -> outer-stage progression onto `EarlyProbeRefinementStages` itself. That leaves the next real seam smaller again: the separate sequence wrapper is gone, but the progression still sits behind a one-use typed `stage_progress()` helper inside the runner.
