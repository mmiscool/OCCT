...
- Completed evidence: `rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/face_snapshot.rs` now uses `ported_snapshot_plane_payload()` for planar multi-wire root-face snapshot reconstruction. That helper requires OCCT geometry only for the surface kind/bounds and then derives the plane through `PortedSurface::from_context_with_ported_payloads()`; the module no longer calls `face_plane_payload()` or `face_plane_payload_occt()`, so supported multi-wire planar topology no longer accepts a direct OCCT plane-payload rescue after Rust extraction fails. `rust/lean_occt/src/occt_port/ModelingData/TKG3d/GeomEval/ported_geometry.rs::ported_face_geometry()` also no longer retries direct analytic `face_plane_payload_occt()`, `face_cylinder_payload_occt()`, `face_cone_payload_occt()`, `face_sphere_payload_occt()`, or `face_torus_payload_occt()` after Rust sample-derived payload extraction fails. `brep_workflows::ported_brep_uses_rust_owned_topology_for_simple_single_face_shapes` now asserts the holed planar face raw-geometry route yields a Rust plane payload and that the BRep face retained the same Rust analytic surface route while preserving loop roles and reconstructed area. Verification passed: `cargo fmt --manifest-path rust/lean_occt/Cargo.toml`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test brep_workflows ported_brep_uses_rust_owned_topology_for_simple_single_face_shapes -- --nocapture`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test ported_geometry_workflows ported_face_surface_descriptors_cover_supported_faces -- --nocapture`, `cargo check --manifest-path rust/lean_occt/Cargo.toml`, `cmake --build build --target LeanOcctCAPI`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test brep_workflows`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test ported_geometry_workflows`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test document_workflows`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test selector_workflows`, `cargo test --manifest-path rust/lean_occt/Cargo.toml`, and `git diff --check`.
- Active milestone: `M7. Public Payload Fallback Narrowing`.
- Next bounded cut: split the public analytic surface payload methods in `rust/lean_occt/src/lib.rs` (`face_plane_payload()`, `face_cylinder_payload()`, `face_cone_payload()`, `face_sphere_payload()`, and `face_torus_payload()`) so supported analytic faces do not call the direct `face_*_payload_occt()` helper when Rust descriptor extraction returns `None`; keep the explicit `*_payload_occt()` APIs available as opt-in oracles.
- Verification:
...
Status: active. The remaining direct surface payload helper calls in source are the public fallback branches in `rust/lean_occt/src/lib.rs` plus tests that use explicit OCCT helpers as parity oracles. The first target family is analytic face payload queries: `face_plane_payload()`, `face_cylinder_payload()`, `face_cone_payload()`, `face_sphere_payload()`, and `face_torus_payload()` still call their direct `face_*_payload_occt()` helper when `ported_face_surface()` returns `None`.
...
Bounded tasks: start with analytic face payload queries in `lib.rs`, split supported-kind failures from unsupported descriptor absence, strengthen public analytic payload coverage to assert the Rust descriptor path is present for exercised faces, then move to swept and offset-basis public payload methods.
