...
- Completed evidence: `rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep.rs` no longer exposes `SummaryBboxSource::OffsetOcctSubshapeUnion`, and `rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs` has deleted `offset_shape_bbox_occt()` plus its `union_shape_bboxes_occt()` helper. Loaded offset-face inventories are now authoritative for `requires_rust_owned_bbox`, so supported offset roots must resolve through `offset_faces_bbox()`, offset validated mesh, or offset solid shell unions instead of falling through to an OCCT subshape bbox union or generic whole-shape summary. The root-level `vertex_shapes` payload and fallback root vertex walk in `ported_brep()` were removed because they only fed the deleted OCCT bbox branch; shell-local prepared vertex inventories remain for shell bbox validation. The exercised single-face offset shell remains on `OffsetFaceBboxSource::ValidatedMesh`, and the exercised multi-face offset-solid shells remain on `OffsetFaceBboxSource::SummaryFaceBrep`. `cargo fmt --manifest-path rust/lean_occt/Cargo.toml`, `cmake --build build --target LeanOcctCAPI`, both focused offset regressions, `cargo check`, the full `brep_workflows` suite, and the full crate `cargo test` suite all passed.
- Active milestone: `M2. Whole-Shape Summary Fallback Reduction`.
- Next bounded cut: narrow the remaining generic `fallback_summary()` bbox and volume branches behind explicit unsupported-shape guards, starting with any loaded analytic/swept/offset face inventory that can already be served by existing Rust-owned bbox or face-contribution paths, while keeping exact-primitive, swept, offset, and topology regressions green.
- Verification:
...
Status: active. The exercised offset-solid bbox path resolves through Rust-owned shell-union data tagged as `SummaryBboxSource::OffsetSolidShellUnion`, the exercised offset-solid volume path resolves through `SummaryVolumeSource::FaceContributions` without touching the shared volume fallback, the exercised swept-revolution solid resolves its root volume through `SummaryVolumeSource::FaceContributions`, the exercised exact-primitive roots now include torus solids on `SummaryBboxSource::ExactPrimitive`, the exercised exact-curve roots stay off the generic whole-shape OCCT bbox fallback, the exercised single-face offset shell root is explicitly observed on `OffsetFaceBboxSource::ValidatedMesh`, and the exercised multi-face offset shell root resolves through `OffsetFaceBboxSource::SummaryFaceBrep`. The old whole-face `face_bboxes_occt()`/`OcctFaceUnion` bbox path and the offset-specific `offset_shape_bbox_occt()`/`OffsetOcctSubshapeUnion` path have both been removed. The remaining gap inside this milestone is the pair of generic `fallback_summary()` bbox and volume branches, which still need explicit unsupported-shape guards.
