...
- Completed evidence: `rust/lean_occt/src/lib.rs` now serves `face_revolution_payload()` and `face_extrusion_payload()` directly from matching `PortedFaceSurface::Swept` descriptors and returns an explicit Rust-owned mismatch error for supported non-matching face descriptors; OCCT fallback remains only when `ported_face_surface_descriptor()` returns `None`. The same strict rule now covers `face_offset_basis_plane_payload()`, `face_offset_basis_cylinder_payload()`, `face_offset_basis_cone_payload()`, `face_offset_basis_sphere_payload()`, `face_offset_basis_torus_payload()`, `face_offset_basis_revolution_payload()`, and `face_offset_basis_extrusion_payload()`: matching `PortedOffsetBasisSurface` payloads return from Rust, supported non-matching basis kinds fail in Rust, and only `None` reaches the OCCT helper. `rust/lean_occt/tests/ported_geometry_workflows.rs` now compares public swept and offset-basis payload APIs against both ported descriptors and OCCT for exercised extrusion/revolution and swept-offset faces, including representative mismatches for top-level swept requests plus plane/cylinder/revolution/extrusion offset-basis requests. `cargo fmt --manifest-path rust/lean_occt/Cargo.toml`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test ported_geometry_workflows`, `cargo check --manifest-path rust/lean_occt/Cargo.toml`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test document_workflows`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test selector_workflows`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test brep_workflows`, and `cargo test --manifest-path rust/lean_occt/Cargo.toml` all passed.
- Active milestone: `M4. Public Query Fallback Cleanup`.
- Next bounded cut: continue `M4` by tightening the remaining offset basis curve fallbacks in `rust/lean_occt/src/lib.rs`. Replace the broad `_ => ..._occt()` branches in `face_offset_basis_curve_geometry()`, `face_offset_basis_curve_line_payload()`, `face_offset_basis_curve_circle_payload()`, and `face_offset_basis_curve_ellipse_payload()` with Rust-owned mismatch errors whenever `ported_offset_surface()` returns a supported basis without the requested swept basis curve or curve kind; keep OCCT fallback only for `None`. Strengthen `ported_geometry_workflows` so exercised swept offset faces prove matching basis curve geometry and ellipse payloads come from Rust descriptors, while representative line/circle mismatch requests fail before OCCT.
- Verification:
...
Status: active. The analytic and swept/offset-basis payload cuts are complete: public line, circle, ellipse, plane, cylinder, cone, sphere, torus, extrusion, revolution, and offset-basis payload APIs now use Rust descriptors for matching supported kinds and return explicit Rust mismatch errors for non-matching supported descriptors. Remaining offset basis curve geometry and curve payload APIs still contain broad `_ => ..._occt()` branches after Rust descriptors identify supported offset/swept basis data; those fallbacks need to become explicit unsupported-kind escape hatches instead of normal continuation paths.
