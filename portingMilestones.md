...
- Completed evidence: `rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/topology.rs::LoadedPortedTopology` now carries the root vertex, edge, wire, face, and prepared shell handles captured during Rust topology loading. `rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/shape_queries.rs` serves `ported_subshape_count()`, `ported_subshape()`, and `ported_subshapes()` for `Vertex`, `Edge`, `Wire`, `Face`, and `Shell` from that loaded Rust topology inventory instead of rematerializing the public subshape list through the generic OCCT helper path. `rust/lean_occt/src/lib.rs` public `vertex_point()` and `topology()` now require Rust `ported_vertex_point()`/`ported_topology()` results, and public `subshape_count()`, `subshape()`, and `subshapes()` fail explicitly for supported topology kinds when Rust inventory extraction returns `None`; the remaining OCCT subshape escape hatch is limited to unsupported shape kinds plus explicit raw `*_occt()` APIs. `rust/lean_occt/tests/brep_workflows.rs`, `document_workflows.rs`, `selector_workflows.rs`, and `ported_geometry_workflows.rs` now compare public topology/traversal/vertex results against Rust-owned topology before using explicit OCCT helpers as oracles.
- Active milestone: `M9. BRep Materialization Fallback Narrowing`.
- Next bounded cut: remove the remaining BRep edge-geometry rescue path that catches a failed Rust/public edge geometry query and silently calls `edge_geometry_occt()` in `rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/brep_materialize.rs::ported_brep_edges()` and `brep/face_topology.rs::single_face_edge_with_route()`. Route supported line/circle/ellipse BRep edge geometry through Rust-owned `PortedCurve`/topology data, keep unsupported curve kinds explicit, and strengthen BRep/document/selector coverage so exercised BRep edge materialization cannot pass by falling back to direct OCCT edge geometry.
- Verification:
  ...
  - `cargo test --manifest-path rust/lean_occt/Cargo.toml --test selector_workflows -- --nocapture`
  - `cargo test --manifest-path rust/lean_occt/Cargo.toml --test brep_workflows -- --nocapture`
  - `cargo test --manifest-path rust/lean_occt/Cargo.toml`
  - `rg -n 'rust_owned_topology_subshape_query_required|unsupported_ported_topology_query_error|ported_subshape_count' rust/lean_occt/src/lib.rs rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep.rs rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/shape_queries.rs`
  - `git diff --check`
...
Status: active. `ported_brep_edges()` in `brep_materialize.rs` and `single_face_edge_with_route()` in `face_topology.rs` still catch `Context::edge_geometry()` failures and call `edge_geometry_occt()`. That can reopen the supported line/circle/ellipse public fallback path narrowed under M8 when BRep materialization is the caller.
...
Bounded tasks: first remove the `Err(_) => context.edge_geometry_occt(edge_shape)?` branches in `ported_brep_edges()` and `single_face_edge_with_route()` for supported edge kinds by routing through a Rust-owned helper that returns either ported geometry or an explicit unsupported-kind error. Then narrow `FaceSurfaceRoute::Raw` face/edge geometry rescue only where the loaded topology route proves the face is supported.

Verification: `cargo fmt --manifest-path rust/lean_occt/Cargo.toml`, `rg -n 'Err\\(_\\) => context\\.edge_geometry_occt|FaceSurfaceRoute::Raw => context\\.(edge|face)_geometry_occt' rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep`, `cargo check --manifest-path rust/lean_occt/Cargo.toml`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test brep_workflows`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test document_workflows`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test selector_workflows`, `cargo test --manifest-path rust/lean_occt/Cargo.toml`.
