...
- Completed evidence: `rust/lean_occt/src/occt_port/ModelingData/TKG3d/GeomEval/ported_geometry.rs::ported_offset_surface()` now enters through `Context::face_geometry(shape)?` before offset reconstruction, and the secondary `Context::ported_offset_face_surface_payload()` descriptor retry in `rust/lean_occt/src/lib.rs` also uses `Context::face_geometry(shape)?` instead of a direct raw geometry read. `public_offset_basis_queries_match_occt` now asserts public, ported, and OCCT face-geometry parity for every exercised offset basis family before public offset payload and basis queries run.
- Active milestone: `M15. Rust-Owned Offset Surface Metadata Extraction`.
- Next bounded cut: replace the exercised `ported_offset_surface_with_geometry()` direct `face_offset_payload_occt(shape)?`/`face_offset_basis_geometry_occt(shape)?` metadata entry for at least the analytic offset-basis family with Rust-owned metadata derived from the already validated public offset geometry and reconstructed basis descriptor, then keep the raw helpers only as explicit oracle APIs or unsupported-basis escape hatches.
- Verification:
  ...
  - `! awk '/pub\(crate\) fn ported_offset_surface/,/^    }/' rust/lean_occt/src/occt_port/ModelingData/TKG3d/GeomEval/ported_geometry.rs | rg -n 'face_geometry_occt\(shape\)'`
  - `! awk '/fn ported_offset_face_surface_payload/,/^    }/' rust/lean_occt/src/lib.rs | rg -n 'face_geometry_occt\(shape\)'`
  - `git diff --check`
...
Status: active. `ported_geometry.rs::ported_offset_surface_with_geometry()` still calls `self.face_offset_payload_occt(shape)?` and `self.face_offset_basis_geometry_occt(shape)?` before reconstructing the Rust offset basis. Those direct helper calls are now the highest-priority OCCT-backed metadata entry for public offset payload, offset-basis, offset sampling, and BRep offset-area paths.

Definition of done: for at least the exercised analytic offset-basis family, `ported_offset_surface_with_geometry()` derives `OffsetSurfacePayload { offset_value, basis_surface_kind }` and the basis `FaceGeometry` through Rust-owned reconstruction from public offset geometry, orientation, and basis descriptor validation, leaving direct `face_offset_payload_occt()` and `face_offset_basis_geometry_occt()` calls only for explicit oracle APIs or unsupported-basis escape hatches. Offset basis queries, offset sampling, and BRep offset-area regressions stay green.

Bounded tasks: start with plane/cylinder/cone/sphere/torus offset faces in `public_offset_basis_queries_match_occt`, introduce a Rust-owned offset metadata candidate that can classify the analytic basis and recover the signed offset distance without calling `face_offset_payload_occt()` or `face_offset_basis_geometry_occt()` on the exercised path, then keep swept offset bases as a named follow-up if the analytic family lands cleanly.
