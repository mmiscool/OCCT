...
- Completed evidence: `rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep.rs` now exposes a public `OffsetFaceBboxSource` probe on `BrepShape`, and `rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs` threads that source through `ported_shape_summary()` while widening `validated_mesh_bbox(...)` to accept the exercised single-face offset-shell mesh candidate after an axis-by-axis `2 * offset_margin + 1e-4` expansion. That moves the exercised single-face offset-shell root off the `face_bboxes_occt()` branch and onto `OffsetFaceBboxSource::ValidatedMesh` without disturbing the already-ported offset-solid root volume path. `rust/lean_occt/tests/brep_workflows.rs` extends `ported_brep_uses_rust_owned_area_for_offset_faces` to assert the new Rust-owned root bbox source, and the focused offset-face regression, the exercised offset-solid regression, the full `brep_workflows` suite, `cargo check`, and the full `cargo test` suite all passed. The remaining blocker inside `M2` is now explicit on the exercised multi-face offset shell: its public source still reports `OffsetFaceBboxSource::OcctFaceUnion` because one plane cap's OCCT face bbox exceeds every current Rust candidate by hundreds of units.
- Active milestone: `M2. Whole-Shape Summary Fallback Reduction`.
- Next bounded cut: use the new `OffsetFaceBboxSource` probe to move the exercised multi-face offset-shell root off `OcctFaceUnion` by deriving a non-recursive Rust bbox candidate for the plane cap that validates the shell-level `SummaryFaceBrep` branch, then delete the remaining multi-face `face_bboxes_occt()` dependency for that exercised family.
- Verification:
...
Status: active. The exercised offset-solid bbox path resolves through Rust-owned shell-union data tagged as `SummaryBboxSource::OffsetSolidShellUnion`, the exercised offset-solid volume path resolves through `SummaryVolumeSource::FaceContributions` without touching the shared volume fallback, the exercised swept-revolution solid resolves its root volume through `SummaryVolumeSource::FaceContributions`, the exercised exact-primitive roots now include torus solids on `SummaryBboxSource::ExactPrimitive`, the exercised exact-curve roots stay off the generic whole-shape OCCT bbox fallback, and the exercised single-face offset shell root is now explicitly observed on `OffsetFaceBboxSource::ValidatedMesh` instead of falling through to `face_bboxes_occt()`. The remaining bbox gap inside this milestone is isolated to the exercised multi-face offset-shell family: its root shell summaries still surface `SummaryBboxSource::OffsetFaceUnion`, but the new public probe shows the winning internal source is still `OffsetFaceBboxSource::OcctFaceUnion` because one plane cap needs a non-recursive Rust bbox candidate that can reach the shell-level summary-face validator. The shared volume fallback remains only for explicitly unsupported families.
