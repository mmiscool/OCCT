...
- Completed evidence: `rust/lean_occt/src/lib.rs` now serves `face_offset_basis_curve_geometry()`, `face_offset_basis_curve_line_payload()`, `face_offset_basis_curve_circle_payload()`, and `face_offset_basis_curve_ellipse_payload()` directly from `PortedOffsetSurface` swept-basis descriptors when the basis curve data matches. Supported analytic offset bases now return explicit Rust-owned "no basis curve" errors for curve geometry/payload requests, supported swept bases with a non-matching curve kind return explicit Rust mismatch errors, and OCCT fallback remains only when `ported_offset_surface()` returns `None`. `rust/lean_occt/tests/ported_geometry_workflows.rs` now asserts exercised extrusion/revolution offset faces compare basis curve geometry and ellipse payloads against ported descriptors and OCCT, and representative line/circle payload requests for an ellipse swept basis fail in Rust before any OCCT helper. `cargo fmt --manifest-path rust/lean_occt/Cargo.toml`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test ported_geometry_workflows public_swept_offset_basis_queries_match_occt`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test ported_geometry_workflows`, `cargo check --manifest-path rust/lean_occt/Cargo.toml`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test document_workflows`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test selector_workflows`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test brep_workflows`, and `cargo test --manifest-path rust/lean_occt/Cargo.toml` all passed.
- Active milestone: `M5. BRep Curve Payload Fallback Cleanup`.
- Next bounded cut: start `M5` by removing the OCCT payload rescue path for supported edge curves in `rust/lean_occt/src/occt_port/ModelingData/TKG3d/GeomEval/ported_geometry.rs`. Make `PortedCurve::from_context_with_ported_payloads()` return Rust-derived line/circle/ellipse payloads when the local extractor succeeds and return `None` for unsupported extraction instead of calling `edge_*_payload_occt()`. Then update the public-route BRep edge materialization call sites in `brep_materialize.rs` and `face_topology.rs` so they stop swallowing Rust extraction errors into `PortedCurve::from_context_with_geometry()`. Strengthen `brep_workflows` around exercised line/circle/ellipse edges so populated `BrepEdge::ported_curve` data and derived lengths stay Rust-owned and match OCCT.
- Verification:
...
Status: active. `PortedCurve::from_context_with_ported_payloads()` still falls back to `edge_line_payload_occt()`, `edge_circle_payload_occt()`, and `edge_ellipse_payload_occt()` when the Rust-owned extractor returns `None`, and public-route BRep call sites in `brep_materialize.rs` and `face_topology.rs` still swallow extraction errors by retrying `PortedCurve::from_context_with_geometry()`.
...
Bounded tasks: remove the OCCT payload rescue from one curve materialization path at a time, starting with `PortedCurve::from_context_with_ported_payloads()` plus the public-route `ported_brep_edges()` and `single_face_edge_with_route()` call sites, then add regression coverage proving exercised analytic edges retain ported curves and parity lengths.
