...
- Completed evidence: `rust/lean_occt/src/lib.rs` now routes the public offset-basis face-surface family (`face_offset_basis_geometry()`, `face_offset_basis_plane_payload()`, `face_offset_basis_cylinder_payload()`, `face_offset_basis_cone_payload()`, `face_offset_basis_sphere_payload()`, `face_offset_basis_torus_payload()`, `face_offset_basis_revolution_payload()`, and `face_offset_basis_extrusion_payload()`) through `ported_offset_face_surface_payload()`. Supported offset faces now require a Rust `PortedOffsetSurface` descriptor before public offset-basis geometry or basis surface payload APIs can succeed; these wrappers no longer call direct `face_offset_basis_*_occt()` helpers when descriptor extraction returns `None`. The explicit `face_offset_basis_*_occt()` APIs remain available as opt-in oracle APIs. `rust/lean_occt/tests/ported_geometry_workflows.rs::public_offset_basis_queries_match_occt` requires matching Rust offset descriptors before public offset-basis geometry/payload calls and still compares the public Rust-owned result against explicit OCCT oracles. Verification passed: `cargo fmt --manifest-path rust/lean_occt/Cargo.toml`, `! rg -n 'None => self.face_offset_basis_(geometry|plane_payload|cylinder_payload|cone_payload|sphere_payload|torus_payload|revolution_payload|extrusion_payload)_occt\\(shape\\)|face_offset_basis_(geometry|plane_payload|cylinder_payload|cone_payload|sphere_payload|torus_payload|revolution_payload|extrusion_payload)_occt\\(shape\\)' rust/lean_occt/src/lib.rs`, `cargo check --manifest-path rust/lean_occt/Cargo.toml`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test ported_geometry_workflows public_offset_basis_queries_match_occt -- --nocapture`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test ported_geometry_workflows ported_face_surface_descriptors_cover_supported_faces -- --nocapture`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test ported_geometry_workflows`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test brep_workflows`, `cargo test --manifest-path rust/lean_occt/Cargo.toml`, and `git diff --check`.
- Active milestone: `M7. Public Payload Fallback Narrowing`.
- Next bounded cut: split the public offset-basis curve family in `rust/lean_occt/src/lib.rs` (`face_offset_basis_curve_geometry()`, `face_offset_basis_curve_line_payload()`, `face_offset_basis_curve_circle_payload()`, and `face_offset_basis_curve_ellipse_payload()`) so supported swept offset bases route through the Rust `PortedOffsetSurface`/`PortedCurve` descriptor and no longer call direct `face_offset_basis_curve_*_occt()` helpers when descriptor extraction returns `None`; keep the explicit `*_occt()` methods as opt-in oracles.
- Verification:
  - `cargo fmt --manifest-path rust/lean_occt/Cargo.toml`
  - `! rg -n 'None => self.face_offset_basis_curve_(geometry|line_payload|circle_payload|ellipse_payload)_occt\\(shape\\)|face_offset_basis_curve_(geometry|line_payload|circle_payload|ellipse_payload)_occt\\(shape\\)' rust/lean_occt/src/lib.rs`
...
Bounded tasks: next split the offset-basis curve query family in `lib.rs`, require `ported_offset_face_surface_payload()` to produce a swept `PortedOffsetSurface` with a matching Rust `PortedCurve` before public `face_offset_basis_curve_geometry()` and curve payload calls can succeed, keep mismatched curve payload requests as explicit Rust errors, and keep `public_offset_basis_queries_match_occt` proving descriptor routing before OCCT oracle comparison.
