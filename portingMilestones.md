...
- Completed evidence: `rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/edge_topology.rs::root_edge_topology()` and `brep/wire_topology.rs::wire_occurrence()` now share `topology_edge_query()`, so topology construction obtains edge geometry and endpoints through the public/Rust-owned `Context::edge_geometry()` and `Context::edge_endpoints()` gates instead of direct `edge_geometry_occt(edge_shape)` / `edge_endpoints_occt(edge_shape)` reads. `brep/wire_topology.rs::root_wire_topology()` now prefers Rust occurrence reconstruction before the raw wire snapshot path, and the raw `topology_occt(&prepared_wire_shape.wire_shape)` fallback is narrowed to duplicate-edge-occurrence wires or occurrence reconstruction misses that the current unique edge inventory cannot represent. `brep/shape_queries.rs::ported_edge_endpoints()` has a root-edge topology seed to avoid re-entering `ported_topology()` during root-edge geometry queries without using direct endpoint C helpers. `rust/lean_occt/tests/brep_workflows.rs` now checks topology edge endpoints and wire vertex chains against public edge queries, while requiring supported line/circle/ellipse edge coverage in exercised topology fixtures.
- Active milestone: `M11. Rust-Owned Single-Face Topology Snapshot`.
- Next bounded cut: replace the active `rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/face_topology.rs::single_face_topology_snapshot()` raw `None => context.topology_occt(face_shape)?` fallback with a supported-face Rust topology requirement, preserving an explicit unsupported/ambiguous escape hatch only where a face cannot yet be reconstructed from ported topology.
- Verification:
  ...
  - `! rg -n 'edge_geometry_occt\\(edge_shape\\)|edge_endpoints_occt\\(edge_shape\\)|edge_geometry_occt\\(local_edge_shape\\)|edge_endpoints_occt\\(local_edge_shape\\)' rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/edge_topology.rs rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/wire_topology.rs`
  - `git diff --check`
...
Status: active. `face_topology.rs::single_face_topology_snapshot()` still has an active `None => context.topology_occt(face_shape)?` fallback. That fallback can silently materialize supported single-face topology from raw OCCT topology after the public Rust topology gate fails.
...
Bounded tasks: inspect `single_face_topology_snapshot()` and its callers, replace the implicit raw fallback with a Rust topology requirement for supported single-face descriptors, preserve an explicit unsupported/ambiguous escape hatch only where ported topology cannot identify the face, and strengthen `brep_workflows` around simple single-face and holed planar topology materialization.

Verification: `cargo fmt --manifest-path rust/lean_occt/Cargo.toml`, `cargo check --manifest-path rust/lean_occt/Cargo.toml`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test brep_workflows ported_brep_uses_rust_owned_topology_for_simple_single_face_shapes -- --nocapture`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test brep_workflows ported_brep_uses_rust_owned_topology -- --nocapture`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test brep_workflows`, `cargo test --manifest-path rust/lean_occt/Cargo.toml`, `! rg -n 'None => context\\.topology_occt\\(face_shape\\)\\?' rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/face_topology.rs`, `git diff --check`.
