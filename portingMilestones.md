...
- Completed evidence: `rust/lean_occt/src/lib.rs` now routes the public edge payload family (`edge_line_payload()`, `edge_circle_payload()`, and `edge_ellipse_payload()`) through `ported_edge_curve_payload()` and Rust `PortedCurve::from_context_with_ported_payloads()`. Supported line/circle/ellipse edges require a Rust `PortedCurve` descriptor before public edge payload APIs can succeed, mismatched public edge payload requests still fail with Rust-owned errors, and these wrappers no longer call direct `edge_*_payload_occt()` helpers when descriptor extraction returns `None`. The explicit `edge_*_payload_occt()` APIs remain available as opt-in oracle APIs. `rust/lean_occt/tests/ported_geometry_workflows.rs::public_analytic_curve_and_surface_payload_queries_match_occt` now requires Rust line/circle/ellipse descriptors before comparing public payloads against explicit OCCT oracles, and it covers line/circle/ellipse mismatch errors. Verification passed: `cargo fmt --manifest-path rust/lean_occt/Cargo.toml`, `! rg -n 'None => self.edge_(line|circle|ellipse)_payload_occt\\(shape\\)|edge_(line|circle|ellipse)_payload_occt\\(shape\\)' rust/lean_occt/src/lib.rs`, `cargo check --manifest-path rust/lean_occt/Cargo.toml`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test ported_geometry_workflows public_analytic_curve_and_surface_payload_queries_match_occt -- --nocapture`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test ported_geometry_workflows ported_curve_sampling_matches_occt -- --nocapture`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test ported_geometry_workflows`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test brep_workflows`, `cargo test --manifest-path rust/lean_occt/Cargo.toml`, `git diff --check`, and `! rg -n 'None => self\\.[A-Za-z0-9_]*payload_occt\\(shape\\)|payload_occt\\(shape\\)' rust/lean_occt/src/lib.rs`.
- Active milestone: `M8. Public Geometry and Sampling Fallback Narrowing`.
- Next bounded cut: split the public edge geometry/sampling family in `rust/lean_occt/src/lib.rs` (`edge_geometry()`, `edge_endpoints()`, `edge_sample()`, and `edge_sample_at_parameter()`) so supported line/circle/ellipse edges require Rust `PortedCurve`/BRep topology descriptors and no longer call direct `edge_*_occt()` helpers when ported extraction returns `None`; keep explicit raw OCCT methods as opt-in oracles.
- Verification:
...
Status: active. After M7, the remaining public fallback branches in `rust/lean_occt/src/lib.rs` are `vertex_point()`, `edge_endpoints()`, `edge_sample()`, `edge_sample_at_parameter()`, `edge_geometry()`, `face_sample()`, `face_sample_normalized()`, `face_geometry()`, `topology()`, `subshape_count()`, `subshape()`, and `subshapes()`.
...
Bounded tasks: start with the public edge geometry/sampling family in `lib.rs`, require Rust `PortedCurve`/BRep topology descriptors before `edge_geometry()`, `edge_endpoints()`, `edge_sample()`, and `edge_sample_at_parameter()` can succeed for supported line/circle/ellipse edges, preserve explicit `*_occt()` methods as oracles, and strengthen `ported_curve_sampling_matches_occt` so public edge geometry/endpoints/samples are compared against Rust descriptors before OCCT oracles.

Verification: `cargo fmt --manifest-path rust/lean_occt/Cargo.toml`, `! rg -n 'None => self.edge_(endpoints|sample|sample_at_parameter|geometry)_occt\\(' rust/lean_occt/src/lib.rs`, `cargo check --manifest-path rust/lean_occt/Cargo.toml`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test ported_geometry_workflows ported_curve_sampling_matches_occt -- --nocapture`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test ported_geometry_workflows public_analytic_curve_and_surface_payload_queries_match_occt -- --nocapture`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test ported_geometry_workflows`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test brep_workflows`, `cargo test --manifest-path rust/lean_occt/Cargo.toml`, `git diff --check`.
