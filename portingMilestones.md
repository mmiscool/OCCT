...
- Completed evidence: `rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/summary.rs` now narrows the remaining multi-face `face_bboxes_occt()` escape hatch behind `offset_faces_require_occt_face_union(...)` instead of letting every multi-face offset-shell summary fall through implicitly. The same file also upgrades `validated_face_brep_bbox(...)` to union face boundary geometry into the face summary and mesh candidates before OCCT validation, plus reuse the axis-aware offset expansion there, so only still-unvalidated faces keep the OCCT face-union path alive. The exercised single-face offset shell regression, the exercised offset-solid shell regression, the full `brep_workflows` suite, `cargo check`, and the full `cargo test` suite all stayed green. The remaining blocker is a mesh-backed plane cap in the exercised multi-face shell family whose Rust face bbox is still far smaller than the OCCT face bbox, so the new guard remains active for that family.
- Active milestone: `M2. Whole-Shape Summary Fallback Reduction`.
- Next bounded cut: add a Rust-owned plane-cap bbox candidate for the exercised mesh-backed cap face so the multi-face offset-shell path stops tripping `offset_faces_require_occt_face_union(...)` and the remaining `face_bboxes_occt()` branch can be deleted for that exercised family.
- Verification:
...
Status: active. The exercised offset-solid bbox path resolves through Rust-owned shell-union data tagged as `SummaryBboxSource::OffsetSolidShellUnion`, the exercised offset-solid volume path resolves through `SummaryVolumeSource::FaceContributions` without touching the shared volume fallback, the exercised swept-revolution solid resolves its root volume through `SummaryVolumeSource::FaceContributions`, the exercised exact-primitive, exact-curve, and single-face offset roots stay off the generic whole-shape OCCT bbox fallback, and the exercised multi-face offset shells still resolve their root shell summaries through `SummaryBboxSource::OffsetFaceUnion`. The remaining bbox gap inside this milestone is now isolated behind the explicit `offset_faces_require_occt_face_union(...)` guard: one mesh-backed plane cap in the exercised multi-face shell family still needs a real Rust bbox candidate before the OCCT-described per-face bbox union can be deleted entirely. The shared volume fallback remains only for explicitly unsupported families.
