...
- Completed evidence: `rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/swept_face.rs::ported_extrusion_face_surface()` and `ported_revolution_face_surface()` now derive non-offset swept BRep descriptor payloads from Rust-owned topology plus face samples instead of `face_extrusion_payload_occt()` and `face_revolution_payload_occt()`. The new swept route derives extrusion direction, revolution axis, basis curve kind, and line/circle/ellipse basis curves from BRep topology, validates descriptor samples against OCCT sampling, and returns `None` for unsupported extraction without retrying direct swept payload helpers. Full `TAU` revolution faces now resolve their basis/sweep parameter tie with the same Rust evaluator convention used by public sampling, keeping document-level revolution descriptors covered. `brep_workflows::ported_brep_summarizes_swept_revolution_solids_in_rust` now checks swept descriptor payload parity, public descriptor parity, sample parity, basis kind, and area stability for the exercised extrusion and revolution BRep faces. Verification passed: `cargo fmt --manifest-path rust/lean_occt/Cargo.toml`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test brep_workflows ported_brep_summarizes_swept_revolution_solids_in_rust -- --nocapture`, `cargo check --manifest-path rust/lean_occt/Cargo.toml`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test ported_geometry_workflows public_swept_and_offset_payload_queries_match_occt -- --nocapture`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test ported_geometry_workflows ported_face_surface_descriptors_cover_supported_faces -- --nocapture`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test document_workflows document_runs_analytic_shape_pipeline -- --nocapture`, `cmake --build build --target LeanOcctCAPI`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test brep_workflows`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test ported_geometry_workflows`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test document_workflows`, `cargo test --manifest-path rust/lean_occt/Cargo.toml --test selector_workflows`, `cargo test --manifest-path rust/lean_occt/Cargo.toml`, and `git diff --check`.
- Active milestone: `M6. BRep Surface Payload Fallback Cleanup`.
- Next bounded cut: remove or strictly narrow the remaining planar multi-wire root-face snapshot fallback in `rust/lean_occt/src/occt_port/ModelingData/TKBRep/BRepTools/brep/face_snapshot.rs` so supported planar face topology no longer accepts `face_plane_payload_occt()` after Rust plane extraction fails.
- Verification:
...
Bounded tasks: analytic BRep face materialization, analytic offset-basis descriptors, swept offset-basis descriptors, and non-offset swept BRep descriptors are complete. Next replace the `brep/face_snapshot.rs` planar multi-wire `face_plane_payload_occt()` acceptance path with Rust-owned plane extraction, then add or strengthen document/BRep coverage for supported multi-wire planar topology so a failed Rust plane payload no longer silently re-enters the OCCT payload helper.
