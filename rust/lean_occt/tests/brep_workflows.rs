...
fn assert_root_shell_public_inventory(
    kernel: &ModelKernel,
    label: &str,
    shape: &Shape,
    topology: &lean_occt::TopologySnapshot,
    ...
    if topology.faces.is_empty() {
        return Err(std::io::Error::other(format!(
            "{label} expected a non-empty root shell face inventory"
        ))
        .into());
    }
    ...
    if shell_shapes.len() != 1 {
        return Err(std::io::Error::other(format!(
            "{label} root shell inventory mismatch: public={} topology=1",
            shell_shapes.len()
        ))
        .into());
    }
    let indexed_shell = kernel.context().subshape(shape, ShapeKind::Shell, 0)?;
    let public_shell_topology = kernel.context().topology_occt(&shell_shapes[0])?;
    let indexed_shell_topology = kernel.context().topology_occt(&indexed_shell)?;
    assert_topology_matches_ignoring_edge_lengths(
        &format!("{label} public root shell handle"),
        &public_shell_topology,
        topology,
    )?;
    assert_topology_matches(
        &format!("{label} indexed root shell handle"),
        &public_shell_topology,
        &indexed_shell_topology,
    )?;

    let face_shapes = kernel.context().subshapes(shape, ShapeKind::Face)?;
    if face_shapes.len() != topology.faces.len() {
        return Err(std::io::Error::other(format!(
            "{label} root shell face handle mismatch: public={} topology={}",
            face_shapes.len(),
            topology.faces.len()
        ))
        .into());
    ...
    for (index, face_shape) in face_shapes.iter().enumerate() {
        let indexed_face = kernel.context().subshape(shape, ShapeKind::Face, index)?;
        let public_face_topology = kernel.context().topology_occt(face_shape)?;
        let indexed_face_topology = kernel.context().topology_occt(&indexed_face)?;
        assert_topology_matches(
            &format!("{label} public root shell face {index} handle"),
            &public_face_topology,
            &indexed_face_topology,
        )?;
        if public_face_topology.faces.len() != 1 {
            return Err(std::io::Error::other(format!(
                "{label} root shell face {index} expected isolated face topology, found {} faces",
                public_face_topology.faces.len()
            ))
            .into());
        }
    }
    ...
    if wire_shapes.len() != topology.wires.len() {
        return Err(std::io::Error::other(format!(
            "{label} root shell wire handle mismatch: public={} topology={}",
            wire_shapes.len(),
            topology.wires.len()
        ))
        .into());
    ...
    for (index, wire_shape) in wire_shapes.iter().enumerate() {
        ...
        let indexed_wire_topology = kernel.context().topology_occt(&indexed_wire)?;
        assert_topology_matches(
            &format!("{label} public root shell wire {index} handle"),
            &public_wire_topology,
            &indexed_wire_topology,
        )?;
        if public_wire_topology.faces.len() != 0 || public_wire_topology.wires.len() != 1 {
            return Err(std::io::Error::other(format!(
                "{label} root shell wire {index} expected isolated wire topology, found wires={} faces={}",
                public_wire_topology.wires.len(),
                public_wire_topology.faces.len()
            ))
            .into());
    ...
    if edge_shapes.len() != topology.edges.len() {
        return Err(std::io::Error::other(format!(
            "{label} root shell edge handle mismatch: public={} topology={}",
            edge_shapes.len(),
            topology.edges.len()
        ))
        .into());
    ...
    for (index, edge_shape) in edge_shapes.iter().enumerate() {
        ...
        let indexed_edge_topology = kernel.context().topology_occt(&indexed_edge)?;
        assert_topology_matches(
            &format!("{label} public root shell edge {index} handle"),
            &public_edge_topology,
            &indexed_edge_topology,
        )?;
        let expected_length = match kernel.context().ported_edge_length(edge_shape)? {
        ...
        if (topology_length - expected_length).abs() > 1.0e-9 {
            return Err(std::io::Error::other(format!(
                "{label} root shell edge {index} length mismatch: topology={topology_length} public={expected_length}"
            ))
            .into());
        }
    }
    ...
    if vertex_shapes.len() != topology.vertex_positions.len() {
        return Err(std::io::Error::other(format!(
            "{label} root shell vertex handle mismatch: public={} topology={}",
            vertex_shapes.len(),
            topology.vertex_positions.len()
        ))
        .into());
    ...
    for (index, vertex_shape) in vertex_shapes.iter().enumerate() {
        ...
        let indexed_vertex_topology = kernel.context().topology_occt(&indexed_vertex)?;
        assert_topology_matches(
            &format!("{label} public root shell vertex {index} handle"),
            &public_vertex_topology,
            &indexed_vertex_topology,
        )?;
        let point = kernel.context().vertex_point(vertex_shape)?;
        assert_vec3_close(
            ...
            topology.vertex_positions[index],
            1.0e-8,
            &format!("{label} root shell vertex {index} public point"),
        )?;
    }
...
fn assert_root_solid_public_inventory(
    kernel: &ModelKernel,
    label: &str,
    shape: &Shape,
    topology: &lean_occt::TopologySnapshot,
    ...
    if topology.faces.is_empty() {
        return Err(std::io::Error::other(format!(
            "{label} expected a non-empty root solid face inventory"
        ))
        .into());
    }
    ...
    let shell_count = kernel.context().subshape_count(shape, ShapeKind::Shell)?;
    let shell_shapes = kernel.context().subshapes(shape, ShapeKind::Shell)?;
    if shell_shapes.is_empty() || shell_shapes.len() != shell_count {
        return Err(std::io::Error::other(format!(
            "{label} root solid shell inventory mismatch: public={} count={shell_count}",
            shell_shapes.len()
        ))
        .into());
    }
    for (index, shell_shape) in shell_shapes.iter().enumerate() {
        ...
        let indexed_shell_topology = kernel.context().topology_occt(&indexed_shell)?;
        assert_topology_matches(
            &format!("{label} public root solid shell {index} handle"),
            &public_shell_topology,
            &indexed_shell_topology,
        )?;
        let shell_label = format!("{label} public shell {index}");
        let shell_rust_topology =
            kernel
                .context()
                .ported_topology(shell_shape)?
                .ok_or_else(|| {
                    std::io::Error::other(format!("expected Rust topology for {shell_label}"))
                })?;
        assert_topology_matches(
            &format!("{shell_label} topology"),
            &shell_rust_topology,
            &public_shell_topology,
        )?;
    ...
    if face_shapes.len() != topology.faces.len() {
        return Err(std::io::Error::other(format!(
            "{label} root solid face handle mismatch: public={} topology={}",
            face_shapes.len(),
            topology.faces.len()
        ))
        .into());
    ...
    for (index, face_shape) in face_shapes.iter().enumerate() {
        ...
        let indexed_face_topology = kernel.context().topology_occt(&indexed_face)?;
        assert_topology_matches(
            &format!("{label} public root solid face {index} handle"),
            &public_face_topology,
            &indexed_face_topology,
        )?;
        if public_face_topology.faces.len() != 1 {
            return Err(std::io::Error::other(format!(
                "{label} root solid face {index} expected isolated face topology, found {} faces",
                public_face_topology.faces.len()
            ))
            .into());
        }
    ...
    if wire_shapes.len() != topology.wires.len() {
        return Err(std::io::Error::other(format!(
            "{label} root solid wire handle mismatch: public={} topology={}",
            wire_shapes.len(),
            topology.wires.len()
        ))
        .into());
    ...
    for (index, wire_shape) in wire_shapes.iter().enumerate() {
        ...
        let indexed_wire_topology = kernel.context().topology_occt(&indexed_wire)?;
        assert_topology_matches(
            &format!("{label} public root solid wire {index} handle"),
            &public_wire_topology,
            &indexed_wire_topology,
        )?;
        if public_wire_topology.faces.len() != 0 || public_wire_topology.wires.len() != 1 {
            return Err(std::io::Error::other(format!(
                "{label} root solid wire {index} expected isolated wire topology, found wires={} faces={}",
                public_wire_topology.wires.len(),
                public_wire_topology.faces.len()
            ))
            .into());
    ...
    if edge_shapes.len() != topology.edges.len() {
        return Err(std::io::Error::other(format!(
            "{label} root solid edge handle mismatch: public={} topology={}",
            edge_shapes.len(),
            topology.edges.len()
        ))
        .into());
    ...
    for (index, edge_shape) in edge_shapes.iter().enumerate() {
        ...
        let indexed_edge_topology = kernel.context().topology_occt(&indexed_edge)?;
        assert_topology_matches(
            &format!("{label} public root solid edge {index} handle"),
            &public_edge_topology,
            &indexed_edge_topology,
        )?;
        let expected_length = match kernel.context().ported_edge_length(edge_shape)? {
        ...
        if (topology_length - expected_length).abs() > 1.0e-9 {
            return Err(std::io::Error::other(format!(
                "{label} root solid edge {index} length mismatch: topology={topology_length} public={expected_length}"
            ))
            .into());
        }
    }
    ...
    if vertex_shapes.len() != topology.vertex_positions.len() {
        return Err(std::io::Error::other(format!(
            "{label} root solid vertex handle mismatch: public={} topology={}",
            vertex_shapes.len(),
            topology.vertex_positions.len()
        ))
        .into());
    ...
    for (index, vertex_shape) in vertex_shapes.iter().enumerate() {
        ...
        let indexed_vertex_topology = kernel.context().topology_occt(&indexed_vertex)?;
        assert_topology_matches(
            &format!("{label} public root solid vertex {index} handle"),
            &public_vertex_topology,
            &indexed_vertex_topology,
        )?;
        let point = kernel.context().vertex_point(vertex_shape)?;
        assert_vec3_close(
            ...
            topology.vertex_positions[index],
            1.0e-8,
            &format!("{label} root solid vertex {index} public point"),
        )?;
    }
...
fn ported_brep_uses_rust_owned_topology_for_simple_multi_face_solids(
    ...
    for (label, shape, expected_face_count) in [
        ...
        assert_topology_backed_subshapes_match(&kernel, label, shape, &rust_topology)?;
        assert_topology_edges_match_public_queries(&kernel, label, shape, &rust_topology, true)?;
        assert_root_solid_public_inventory(&kernel, label, shape, &rust_topology)?;
        assert_summary_backed_subshape_counts_match(&kernel, label, shape)?;
        assert_topology_matches(label, &brep.topology, &rust_topology)?;
        assert_brep_edge_geometries_match_public(&kernel, label, shape, &brep)?;
        assert_brep_faces_match_public(&kernel, label, shape, &brep)?;
        ...
        for (shell_index, shell_shape) in shell_shapes.iter().enumerate() {
            let shell_label = format!("{label} shell {shell_index}");
            let shell_topology =
                kernel
                    .context()
                    .ported_topology(shell_shape)?
            ...
            let shell_occt_topology = kernel.context().topology_occt(shell_shape)?;
            let shell_brep = kernel.context().ported_brep(shell_shape)?;
            assert_topology_matches(&shell_label, &shell_topology, &shell_occt_topology)?;
            assert_root_shell_public_inventory(
                &kernel,
                &shell_label,
                shell_shape,
            ...
            assert_topology_matches(
                &format!("{shell_label} BRep topology"),
                &shell_brep.topology,
                &shell_topology,
            )?;
        }
...
#[test]
fn ported_brep_uses_rust_owned_topology_for_root_compound_solids(
) -> Result<(), Box<dyn std::error::Error>> {
    let _guard = support::test_guard();
    let kernel = ModelKernel::new()?;
    ...
    let compound = kernel.fuse(&lhs, &rhs)?;
    let occt_summary = kernel.context().describe_shape_occt(&compound)?;
    assert_eq!(occt_summary.root_kind, ShapeKind::Compound);
    assert_eq!(occt_summary.compound_count, 1);
    assert_eq!(occt_summary.compsolid_count, 0);
    assert_eq!(occt_summary.solid_count, 2);
    assert_eq!(occt_summary.shell_count, 2);
    assert_eq!(occt_summary.face_count, 12);

    let rust_topology = kernel
        .context()
        .ported_topology(&compound)?
        .ok_or_else(|| std::io::Error::other("expected Rust topology for root compound"))?;
    let occt_topology = kernel.context().topology_occt(&compound)?;
    let brep = kernel.brep(&compound)?;

    assert_topology_matches("root compound", &rust_topology, &occt_topology)?;
    assert_topology_backed_subshape_counts_match(
        &kernel,
        "root compound",
        &compound,
        &rust_topology,
    )?;
    assert_topology_backed_subshapes_match(&kernel, "root compound", &compound, &rust_topology)?;
    assert_summary_backed_subshape_counts_match(&kernel, "root compound", &compound)?;
    assert_topology_matches(
        "root compound BRep topology",
        &brep.topology,
        &rust_topology,
    )?;
    assert_brep_edge_geometries_match_public(&kernel, "root compound", &compound, &brep)?;
    assert_brep_faces_match_public(&kernel, "root compound", &compound, &brep)?;
    assert_brep_edge_lengths_match(&kernel, "root compound", &compound, &brep)?;

    let solid_shapes = kernel.context().subshapes(&compound, ShapeKind::Solid)?;
    let occt_solid_shapes = kernel
        .context()
        .subshapes_occt(&compound, ShapeKind::Solid)?;
    if solid_shapes.len() != occt_summary.solid_count
        || solid_shapes.len() != occt_solid_shapes.len()
    {
        return Err(std::io::Error::other(format!(
            "root compound solid inventory mismatch: public={} summary={} occt={}",
            solid_shapes.len(),
    ...
    {
        let indexed_solid = kernel
            .context()
            .subshape(&compound, ShapeKind::Solid, index)?;
        ...
        assert_topology_matches(
            &format!("root compound public solid {index} handle"),
            &public_solid_topology,
            &occt_solid_topology,
        )?;
        assert_topology_matches(
            &format!("root compound indexed solid {index} handle"),
        ...
        let solid_rust_topology =
            kernel
                ...
                .ok_or_else(|| {
                    std::io::Error::other(format!(
                        "expected Rust topology for root compound solid {index}"
                    ))
                })?;
        assert_topology_matches(
            &format!("root compound solid {index} topology"),
            &solid_rust_topology,
            &occt_solid_topology,
        )?;
        assert_root_solid_public_inventory(
            &kernel,
            &format!("root compound solid {index}"),
            solid_shape,
            &solid_rust_topology,
        )?;
    }
    ...
    assert!(
        ...
        "root compound surface area mismatch: rust={} occt={}",
        rust_summary.surface_area,
        occt_summary.surface_area
    );
    assert!(
        (rust_summary.volume - occt_summary.volume).abs() <= 1.0e-8,
        "root compound volume mismatch: rust={} occt={}",
        rust_summary.volume,
        occt_summary.volume
    );
