...
fn offset_faces_bbox(
    ...
    if let Some(resolution) = single_offset_face_surface_resolution(
        context,
        shape_occt_bbox,
        offset_margin,
        vertices,
...
fn single_offset_face_surface_resolution(
    context: &Context,
    ...
    let bbox = validated_offset_face_union_bbox(bbox, expected_bbox, offset_margin, true)?;
    Some(OffsetFaceBrepUnionResolution {
        bbox,
        source: OffsetFaceBboxSource::SummaryFaceBrep,
    })
}
...
fn validated_face_brep_bbox(context: &Context, face_shape: &Shape) -> Option<([f64; 3], [f64; 3])> {
    ...
    let brep = context.ported_brep(face_shape).ok()?;
    let boundary_bbox = boundary_shape_bbox(&brep.vertices, &brep.edges);
    let offset_margin = offset_brep_margin(&brep);
    let summary_bbox = merge_optional_bbox(
        (brep.summary.bbox_min, brep.summary.bbox_max),
        boundary_bbox,
    );
    ...
    if let Some(offset_margin) = offset_margin {
        let expanded_bbox =
            expand_offset_bbox_toward_expected(summary_bbox, face_occt_bbox, offset_margin);
    ...
    if let Some(mesh_bbox) = mesh_bbox {
        if bbox_matches(mesh_bbox, face_occt_bbox) {
            return Some(mesh_bbox);
        }

        if let Some(offset_margin) = offset_margin {
            let expanded_bbox =
                expand_offset_bbox_toward_expected(mesh_bbox, face_occt_bbox, offset_margin);
            if bbox_matches(expanded_bbox, face_occt_bbox) {
                return Some(expanded_bbox);
            }
        }
...
fn merge_optional_bbox(
    bbox: ([f64; 3], [f64; 3]),
    extra_bbox: Option<([f64; 3], [f64; 3])>,
) -> ([f64; 3], [f64; 3]) {
    match extra_bbox {
        Some(extra_bbox) => union_bbox(bbox, extra_bbox),
        None => bbox,
    }
}
...
impl<const SOURCE_N: usize, const STAGE_N: usize> EarlyProbeStageLayout<SOURCE_N, STAGE_N> {
    ...
    fn stage_samples_or_refinement(
        ...
        edge_shape: &Shape,
        source: [NormalizedEdgeSample; SOURCE_N],
    ) -> EarlyProbeStageResult<STAGE_N> {
        let probes = self
            .probe_request_layout
            .probe_pair(context, edge_shape, source);
        EarlyProbeStageResult(match probes {
            Some(probes) => probes
...
impl<const STAGE_N: usize> EarlyProbeStageResult<STAGE_N> {
    fn continue_with<const NEXT_STAGE_N: usize>(
        self,
        next_stage: EarlyProbeStageLayout<STAGE_N, NEXT_STAGE_N>,
        context: &Context,
        edge_shape: &Shape,
    ) -> EarlyProbeStageResult<NEXT_STAGE_N> {
        EarlyProbeStageResult(self.0.and_then(|samples| {
            next_stage
                .stage_samples_or_refinement(context, edge_shape, samples)
                .0
        }))
    }
}
...
impl EarlyProbeStageResult<7> {
    fn needs_refinement(
        ...
        match self.0 {
            Ok(samples) => tail.needs_refinement(samples, context, edge_shape),
            Err(result) => result,
        }
    }
}
...
const EARLY_PROBE_INTERVAL_AWARE_TAIL: EarlyProbeIntervalAwareTail =
