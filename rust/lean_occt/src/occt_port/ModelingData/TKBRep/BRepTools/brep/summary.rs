...
fn sampled_edge_interval_needs_probe_refinement(
    ...
    EARLY_PROBE_REFINEMENT_PIPELINE.needs_refinement(context, edge_shape, start, midpoint, end)
}
...
impl<const SOURCE_N: usize, const STAGE_N: usize> EarlyProbeStageLayout<SOURCE_N, STAGE_N> {
    ...
    fn refinement_result(
        ...
        self.refinement_result_from_source(
            context,
            edge_shape,
            EarlyProbeStageSource::Array(source),
        )
    }

    fn refinement_result_from_source(
        &self,
        context: &Context,
        edge_shape: &Shape,
        source: EarlyProbeStageSource<'_, SOURCE_N>,
    ) -> Option<Result<[NormalizedEdgeSample; STAGE_N], bool>> {
...
#[derive(Clone, Copy)]
enum EarlyProbeStageSource<'a, const SOURCE_N: usize> {
    Array(&'a [NormalizedEdgeSample; SOURCE_N]),
    Triplet(EarlyProbeTripletSource<'a>),
}

impl<const SOURCE_N: usize> EarlyProbeStageSource<'_, SOURCE_N> {
    fn source_sample(self, index: usize) -> NormalizedEdgeSample {
        match self {
            EarlyProbeStageSource::Array(source) => source[index],
            EarlyProbeStageSource::Triplet(source) => source.source_sample(index),
        }
    }
}

#[derive(Clone, Copy)]
struct EarlyProbeTripletSource<'a> {
    start: &'a NormalizedEdgeSample,
    midpoint: &'a NormalizedEdgeSample,
    end: &'a NormalizedEdgeSample,
}

impl<'a> EarlyProbeTripletSource<'a> {
    const fn new(
        start: &'a NormalizedEdgeSample,
        midpoint: &'a NormalizedEdgeSample,
        end: &'a NormalizedEdgeSample,
...
impl EarlyProbeRefinementTerminal {
    const fn new(
        interval_aware_side_layouts: PreparedIntervalAwareRefinementSideLayouts,
        coarse_refinement_checks_before_adaptive_chase: usize,
    ) -> Self {
    ...
    fn needs_refinement(
        ...
        context: &Context,
        edge_shape: &Shape,
        outer_samples: &[NormalizedEdgeSample; 7],
    ) -> Option<bool> {
...
#[derive(Clone, Copy)]
struct EarlyProbeRefinementPipeline {
    stages: EarlyProbeRefinementStages,
    terminal: EarlyProbeRefinementTerminal,
}

impl EarlyProbeRefinementPipeline {
    const fn new(
        stages: EarlyProbeRefinementStages,
        terminal: EarlyProbeRefinementTerminal,
    ) -> Self {
    ...
    fn needs_refinement(
        ...
        let midpoint_source = EarlyProbeTripletSource::new(start, midpoint, end);
        self.stages
            .needs_refinement(context, edge_shape, midpoint_source, self.terminal)
    }
}
...
const MIDPOINT_EARLY_PROBE_STAGE_LAYOUT: EarlyProbeStageLayout<3, 5> = EarlyProbeStageLayout::new(
    ...
    [
        ...
        EarlyProbeSampleRole::Source(1),
        EarlyProbeSampleRole::SecondProbe,
        EarlyProbeSampleRole::Source(2),
    ],
);
...
const OUTER_EARLY_PROBE_STAGE_LAYOUT: EarlyProbeStageLayout<5, 7> = EarlyProbeStageLayout::new(
    ...
    [
        ...
        EarlyProbeSampleRole::Source(1),
        EarlyProbeSampleRole::Source(2),
        EarlyProbeSampleRole::Source(3),
        EarlyProbeSampleRole::SecondProbe,
        EarlyProbeSampleRole::Source(4),
    ],
);
...
const EARLY_PROBE_REFINEMENT_PIPELINE: EarlyProbeRefinementPipeline =
    EarlyProbeRefinementPipeline::new(
        EARLY_PROBE_REFINEMENT_STAGES,
        EarlyProbeRefinementTerminal::new(PREPARED_INTERVAL_AWARE_REFINEMENT_SIDE_LAYOUTS, 3),
    );
